...
--[[
	...
	Custom-built remote spy (no external loadstring). Hooks __namecall to log
	RemoteEvent/RemoteFunction calls and shows generated calling code in a
	Dex+++ styled window.
]]
...
local function main()
	...
	local function generateCode(entry)
		...
		local header = ("-- Remote Spy | %s\n-- Remote: %s\n-- Method: %s\n-- Called by: %s\n-- Time: %s\n\n"):format(
			entry.ClassName, remote and remote.Name or "?", method, entry.Caller or "unknown", os.date("%H:%M:%S", entry.Time)
		)
		...
		local function makeListEntry(entry)
			...
			nameLabel.Text = entry.Remote and entry.Remote.Name or "?"
			...
			local isFunc = (entry.Method == "InvokeServer" or entry.Method == "invokeServer")
			infoLabel.Text = (isFunc and "RemoteFunction - " or "RemoteEvent - ")..entry.ArgCount.." arg(s) - "..os.date("%H:%M:%S", entry.Time)
	...
	local function setupHook()
		...
		oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
			...
			if cls and typeof(self) == "Instance" then
				...
				if not paused and not ignored[self] then
					local args = {...}
					local n = select("#", ...)
	...
	RemoteSpy.Init = function()
		...
		codeFrame.Frame.Parent = window.GuiElems.Content
		codeFrame:SetText("-- Remote Spy ready.\n-- Fire a remote in-game and it will appear on the left.\n-- Click an entry to view its generated calling code.")
