...
local function main()
	...
	-- Use plain TextLabel for display without syntax highlighting
...
local function main()
	...
	pcall(function()
		...
		local function checkForAudio()
			local descendants = workspace:GetDescendants()
			for _, obj in pairs(descendants) do
				if obj:IsA("Sound") then
					if obj.Playing then
						local parent = obj.Parent
						local source = parent and parent.Name or "Unknown"
						local audioInfo = "SoundId: " .. (obj.SoundId or "None") .. " | Volume: " .. tostring(math.floor(obj.Volume * 100)) .. "%"
		...
		-- Connect to DescendantAdded
		workspace.DescendantAdded:Connect(function(descendant)
			if descendant:IsA("Sound") then
				if descendant.Playing then
					local source = descendant.Parent and descendant.Parent.Name or "Unknown"
					local audioInfo = "SoundId: " .. (descendant.SoundId or "None") .. " | Volume: " .. tostring(math.floor(descendant.Volume * 100)) .. "%"
					AudioLogger.LogAudio(source, audioInfo)
				end
				
				descendant.Playing:Connect(function()
					if descendant.Playing then
						local source = descendant.Parent and descendant.Parent.Name or "Unknown"
						local audioInfo = "SoundId: " .. (descendant.SoundId or "None") .. " | Volume: " .. tostring(math.floor(descendant.Volume * 100)) .. "%"
						AudioLogger.LogAudio(source, audioInfo)
...
local Main, Lib, Apps, Settings
local Explorer, Properties, ScriptViewer, SettingsWindow
local API, RMD, env, service, plr, create, createSimple
...
Main = (function()
	...
	Main.Init = function()
		...
		Main.LargeIcons = Lib.IconMap.new("rbxassetid://110441470973171",256,256,32,32)
