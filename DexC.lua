--[[
	...
	DexC is the revival of Chillz's Dex++ and Moon's Dex.
]]
...
local function main()
	...
	ConsoleFrame["BackgroundColor3"] = Color3.fromRGB(41, 45, 53);
	...
	G2L["3"]["BackgroundColor3"] = Color3.fromRGB(33, 37, 44);
	...
	G2L["5"]["ScrollBarImageColor3"] = Color3.fromRGB(52, 57, 68);
	...
	G2L["6"]["PlaceholderText"] = [[Run Lua or a command (type 'cmds')]];
	...
	G2L["backgroundOutput"]["BackgroundColor3"] = Color3.fromRGB(31, 34, 42);
	...
	G2L["a"]["TopImage"] = '';
	G2L["a"]["BackgroundColor3"] = Color3.fromRGB(31, 34, 42);
	...
	G2L["a"].ScrollBarImageColor3 = Color3.fromRGB(64, 70, 82)
	...
	G2L["10"]["BackgroundColor3"] = Color3.fromRGB(33, 37, 44);
	...
	G2L["14"]["BackgroundColor3"] = Color3.fromRGB(52, 57, 68);
	...
	G2L["19"]["BackgroundColor3"] = Color3.fromRGB(52, 57, 68);
	...
	G2L["20"]["BackgroundColor3"] = Color3.fromRGB(52, 57, 68);
	...
	Console.Init = function()
		...
		if CtrlScroll == true then
			Console.CtrlScroll.BackgroundColor3 = Settings.Theme.ListSelection
		elseif CtrlScroll == false then
			Console.CtrlScroll.BackgroundColor3 = Settings.Theme.Button
		end
		...
		if AutoScroll == true then
			Console.AutoScroll.BackgroundColor3 = Settings.Theme.ListSelection
		elseif AutoScroll == false then
			Console.AutoScroll.BackgroundColor3 = Settings.Theme.Button
		end
		...
		-- Admin Terminal (client-side commands)
		local runAdminCommand
		do
			local Workspace = game:GetService("Workspace")
			local Lighting = game:GetService("Lighting")
			local State = {
				Fly = false, FlySpeed = 50, FlyConns = {}, FlyBV = nil, FlyBG = nil,
				Noclip = false, NoclipConn = nil,
				Aimbot = false, AimbotConn = nil, AimbotToggleKey = Enum.KeyCode.Q, AimTeamCheck = false, AimFov = 0, AimSmooth = 0, AimPart = "Head",
				Orbit = false, OrbitConn = nil, OrbitSpeed = 1, OrbitRadius = 10, OrbitAngle = 0, OrbitTarget = nil,
				InfJump = false, InfJumpConn = nil,
				Follow = false, FollowConn = nil, FollowTarget = nil, FollowDist = 5,
				Spectate = false, SpectateTarget = nil,
				Freecam = false, FreecamConns = {}, FreecamCFrame = nil, FreecamSpeed = 1,
				ESP = false, ESPConn = nil, ESPObjects = {},
				Fullbright = false, FbStore = nil,
				AntiAfk = false, AntiAfkConn = nil,
				Spin = false, SpinConn = nil, SpinSpeed = 20, SpinBG = nil,
				God = false, GodConn = nil,
				WalkSpeed = nil, JumpPower = nil, HipHeight = nil, RespawnConn = nil,
				LoopHeal = false, LoopHealConn = nil,
				Attach = false, AttachConn = nil, AttachTarget = nil,
				Waypoints = {},
			}

			-- Direct echo into console output (reliable, not dependent on print hooks)
			local function echo(text, color)
				local ok = pcall(function()
					local tmpl = Console:FindFirstChild("OutputTemplate")
					local out = Console:FindFirstChild("Output")
					if not tmpl or not out then error("no output") end
					local line = tmpl:Clone()
					local stamp = os.date("%H:%M:%S")
					-- escape rich-text special chars so <n>/<user>/& don't break coloring
					local safe = tostring(text):gsub("&", "&amp;"):gsub("<", "&lt;"):gsub(">", "&gt;")
					line.Text = stamp.."   <font color=\""..(color or "rgb(120,220,160)").."\">"..safe.."</font>"
					local ts = out:FindFirstChild("OutputTextSize")
					line.TextSize = ts and ts.Value or 14
					line.Parent = out
					line.Visible = true
					out.CanvasPosition = Vector2.new(0, 9e9)
				end)
			...
			local function findPlayer(name)
				...
				for _, p in ipairs(Players:GetPlayers()) do
					if p ~= LocalPlayer then
						...
						if un == name or dn == name then return p end
						if un:sub(1,#name) == name or dn:sub(1,#name) == name then
							if not best or #un < bestLen then best, bestLen = p, #un end
						end
					end
				end
				return best
			...
			-- Re-apply humanoid stats on respawn
			local function ensureRespawnHook()
				if State.RespawnConn then return end
				State.RespawnConn = LocalPlayer.CharacterAdded:Connect(function()
					task.wait(0.4)
					local _, hum = getChar()
					if not hum then return end
					if State.WalkSpeed then hum.WalkSpeed = State.WalkSpeed end
					if State.JumpPower then pcall(function() hum.UseJumpPower = true end) pcall(function() hum.JumpPower = State.JumpPower end) pcall(function() hum.JumpHeight = State.JumpPower end) end
			...
			local function stopFly()
				State.Fly = false
				for _, c in ipairs(State.FlyConns) do pcall(function() c:Disconnect() end) end
				State.FlyConns = {}
			...
			local function startFly()
				...
				if not hrp then echo("Fly: no character") return end
				stopFly()
				State.Fly = true
				...
				bv.Velocity = Vector3.zero; bv.MaxForce = Vector3.new(9e9,9e9,9e9); bv.P = 9e4
				bv.Parent = hrp
				State.FlyBG, State.FlyBV = bg, bv
				local keys = {}
				State.FlyConns[#State.FlyConns+1] = UserInputService.InputBegan:Connect(function(i, gp)
					if gp then return end
					keys[i.KeyCode] = true
				end)
				State.FlyConns[#State.FlyConns+1] = UserInputService.InputEnded:Connect(function(i)
					keys[i.KeyCode] = nil
				end)
				State.FlyConns[#State.FlyConns+1] = RunService.RenderStepped:Connect(function()
					...
					if keys[K.W] then dir = dir + look end
					if keys[K.S] then dir = dir - look end
					if keys[K.D] then dir = dir + right end
					if keys[K.A] then dir = dir - right end
					if keys[K.Space] then dir = dir + Vector3.new(0,1,0) end
					if keys[K.LeftShift] or keys[K.LeftControl] then dir = dir - Vector3.new(0,1,0) end
					bg.CFrame = cam.CFrame
					if dir.Magnitude > 0 then bv.Velocity = dir.Unit * State.FlySpeed else bv.Velocity = Vector3.zero end
				end)
			end
			...
			local function getClosestToMouse()
				...
				for _, p in ipairs(Players:GetPlayers()) do
					if p ~= LocalPlayer then
						if State.AimTeamCheck and p.Team == LocalPlayer.Team then continue end
						local ch = p.Character
						local part = ch and (ch:FindFirstChild(State.AimPart) or ch:FindFirstChild("Head") or ch:FindFirstChild("HumanoidRootPart"))
						local hum = ch and ch:FindFirstChildOfClass("Humanoid")
						if part and hum and hum.Health > 0 then
							local sp, onScreen = cam:WorldToViewportPoint(part.Position)
							if onScreen then
								local dist = (Vector2.new(sp.X, sp.Y) - mpos).Magnitude
								if State.AimFov > 0 and dist > State.AimFov then continue end
								if not closestDist or dist < closestDist then closest, closestDist = part, dist end
							end
						end
					end
				end
			...
			local function startAimbot()
				...
				State.AimbotConn = RunService.RenderStepped:Connect(function()
					...
					if target then
						...
						if State.AimSmooth and State.AimSmooth > 0 then
							cam.CFrame = cam.CFrame:Lerp(goal, 1 / (State.AimSmooth + 1))
						else
							cam.CFrame = goal
						end
					end
			...
			local function stopOrbit()
				...
				if State.OrbitConn then State.OrbitConn:Disconnect() State.OrbitConn = nil end
			end
			local function startOrbit(target)
				local _,_,hrp = getChar()
				if not hrp then echo("Orbit: no character") return end
				stopOrbit()
				State.Orbit = true
				State.OrbitTarget = target
				State.OrbitAngle = 0
				State.OrbitConn = RunService.RenderStepped:Connect(function(dt)
					local tch = target.Character
					local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
					local _,_,myhrp = getChar()
					if not thrp or not myhrp then return end
					State.OrbitAngle = State.OrbitAngle + dt * State.OrbitSpeed * 2
					local center = thrp.Position
					local x = math.cos(State.OrbitAngle) * State.OrbitRadius
					local z = math.sin(State.OrbitAngle) * State.OrbitRadius
					myhrp.CFrame = CFrame.new(center + Vector3.new(x, 0, z), center)
				end)
			end
			...
			local function stopFollow()
				...
				if State.FollowConn then State.FollowConn:Disconnect() State.FollowConn = nil end
			end
			local function startFollow(target)
				...
				State.FollowConn = RunService.Heartbeat:Connect(function()
					local tch = target.Character
					local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
					local _, hum, myhrp = getChar()
					if not thrp or not myhrp or not hum then return end
					if (thrp.Position - myhrp.Position).Magnitude > State.FollowDist then
			...
			local function startSpectate(target)
				...
				if not thum then echo("Spectate: target has no character") return end
				State.Spectate = true
			...
			local function stopFreecam()
				State.Freecam = false
				for _, c in ipairs(State.FreecamConns) do pcall(function() c:Disconnect() end) end
				State.FreecamConns = {}
				local cam = Workspace.CurrentCamera
			...
			local function startFreecam()
				...
				local pitch, yaw = 0, 0
				do
					local _, ry = cam.CFrame:ToOrientation()
					yaw = ry
				end
				State.FreecamConns[#State.FreecamConns+1] = UserInputService.InputBegan:Connect(function(i, gp)
					if gp then return end
					keys[i.KeyCode] = true
				end)
				State.FreecamConns[#State.FreecamConns+1] = UserInputService.InputEnded:Connect(function(i)
					keys[i.KeyCode] = nil
				end)
				State.FreecamConns[#State.FreecamConns+1] = UserInputService.InputChanged:Connect(function(i, gp)
					if i.UserInputType == Enum.UserInputType.MouseMovement and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
						yaw = yaw - i.Delta.X * 0.005
						pitch = math.clamp(pitch - i.Delta.Y * 0.005, -1.4, 1.4)
					end
				end)
				State.FreecamConns[#State.FreecamConns+1] = RunService.RenderStepped:Connect(function(dt)
					if not State.Freecam then return end
					local cf = State.FreecamCFrame
					local rot = CFrame.fromEulerAnglesYXZ(pitch, yaw, 0)
					local pos = cf.Position
					local move = Vector3.zero
					local K = Enum.KeyCode
					local sp = 60 * State.FreecamSpeed * dt
					if keys[K.W] then move = move + rot.LookVector end
					if keys[K.S] then move = move - rot.LookVector end
					if keys[K.D] then move = move + rot.RightVector end
					if keys[K.A] then move = move - rot.RightVector end
					if keys[K.E] or keys[K.Space] then move = move + Vector3.new(0,1,0) end
					if keys[K.Q] or keys[K.LeftControl] then move = move - Vector3.new(0,1,0) end
					pos = pos + move * sp
					State.FreecamCFrame = CFrame.new(pos) * rot
					cam.CFrame = State.FreecamCFrame
				end)
			end
			...
			local function makeESPFor(p)
				...
				hl.Adornee = char
				hl.Parent = char
				State.ESPObjects[#State.ESPObjects+1] = hl
				...
				if head then
					...
					lbl.Parent = bb
					bb.Parent = head
					State.ESPObjects[#State.ESPObjects+1] = bb
				end
			end
			...
			local function startESP()
				...
				State.ESPConn = Players.PlayerAdded:Connect(function(p)
					p.CharacterAdded:Connect(function() task.wait(0.5) if State.ESP then makeESPFor(p) end end)
				end)
				for _, p in ipairs(Players:GetPlayers()) do
					if p ~= LocalPlayer then
						p.CharacterAdded:Connect(function() task.wait(0.5) if State.ESP then makeESPFor(p) end end)
			...
			local function stopFullbright()
				...
				if State.FbStore then
					pcall(function()
						Lighting.Brightness = State.FbStore.Brightness
						Lighting.ClockTime = State.FbStore.ClockTime
						Lighting.FogEnd = State.FbStore.FogEnd
						Lighting.GlobalShadows = State.FbStore.GlobalShadows
						Lighting.Ambient = State.FbStore.Ambient
						Lighting.OutdoorAmbient = State.FbStore.OutdoorAmbient
					end)
					State.FbStore = nil
				end
			end
			local function startFullbright()
				if not State.FbStore then
					State.FbStore = {
			...
			local function stopSpin()
				...
				if State.SpinBG then State.SpinBG:Destroy() State.SpinBG = nil end
			end
			local function startSpin()
				local _,_,hrp = getChar()
				if not hrp then echo("Spin: no character") return end
				stopSpin()
				State.Spin = true
				local bg = Instance.new("BodyAngularVelocity")
				bg.MaxTorque = Vector3.new(0, 9e9, 0)
				bg.AngularVelocity = Vector3.new(0, State.SpinSpeed, 0)
				bg.P = 9e4
				bg.Parent = hrp
				State.SpinBG = bg
				State.SpinConn = RunService.Heartbeat:Connect(function()
					if State.SpinBG then State.SpinBG.AngularVelocity = Vector3.new(0, State.SpinSpeed, 0) end
				end)
			end
			...
			local Commands = {}
			local Order = {}
			local function reg(names, help, fn)
				local entry = {help = help, fn = fn, primary = names[1], names = names}
				Order[#Order+1] = entry
				for _, n in ipairs(names) do Commands[n:lower()] = entry end
			end

			-- MOVEMENT
			reg({"fly"}, "fly [speed] - toggle flight (WASD + Space/Shift)", function(a)
				if a[1] then State.FlySpeed = tonumber(a[1]) or State.FlySpeed end
				if State.Fly then stopFly() echo("Fly: OFF") else startFly() echo("Fly: ON (speed "..State.FlySpeed..")") end
			end)
			reg({"unfly","nofly"}, "unfly - turn off flight", function() stopFly() echo("Fly: OFF") end)
			reg({"flyspeed","fspeed"}, "flyspeed <n> - set fly speed", function(a)
				State.FlySpeed = tonumber(a[1]) or State.FlySpeed; echo("Fly speed: "..State.FlySpeed)
			end)
			reg({"noclip","nc"}, "noclip - toggle walk through walls", function()
				if State.Noclip then stopNoclip() echo("Noclip: OFF") else startNoclip() echo("Noclip: ON") end
			end)
			reg({"unnoclip","clip","unnc"}, "unnoclip - turn off noclip", function() stopNoclip() echo("Noclip: OFF") end)
			reg({"infjump","infinitejump","ij"}, "infjump - toggle infinite jump", function()
				if State.InfJump then stopInfJump() echo("Infinite Jump: OFF") else startInfJump() echo("Infinite Jump: ON") end
			end)
			reg({"uninfjump","noinfjump","unij"}, "uninfjump - turn off infinite jump", function() stopInfJump() echo("Infinite Jump: OFF") end)
			reg({"walkspeed","ws","speed"}, "walkspeed <n> - set walk speed", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: walkspeed <n>") return end
				State.WalkSpeed = n; ensureRespawnHook()
				local _, hum = getChar(); if hum then hum.WalkSpeed = n end
				echo("WalkSpeed: "..n)
			end)
			reg({"unwalkspeed","unws","resetspeed"}, "unwalkspeed - reset walk speed to 16", function()
				State.WalkSpeed = nil
				local _, hum = getChar(); if hum then hum.WalkSpeed = 16 end
				echo("WalkSpeed reset (16)")
			end)
			reg({"jumppower","jp","jump"}, "jumppower <n> - set jump power/height", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: jumppower <n>") return end
				State.JumpPower = n; ensureRespawnHook()
				local _, hum = getChar()
				if hum then pcall(function() hum.UseJumpPower = true end) pcall(function() hum.JumpPower = n end) pcall(function() hum.JumpHeight = n end) end
				echo("JumpPower: "..n)
			end)
			reg({"unjumppower","unjp","resetjump"}, "unjumppower - reset jump power to 50", function()
				State.JumpPower = nil
				local _, hum = getChar()
				if hum then pcall(function() hum.JumpPower = 50 end) pcall(function() hum.JumpHeight = 7.2 end) end
				echo("JumpPower reset (50)")
			end)
			reg({"hipheight","hh"}, "hipheight <n> - set hip height", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: hipheight <n>") return end
				State.HipHeight = n; ensureRespawnHook()
				local _, hum = getChar(); if hum then hum.HipHeight = n end
				echo("HipHeight: "..n)
			end)
			reg({"unhipheight","unhh","resethip"}, "unhipheight - reset hip height to 0", function()
				State.HipHeight = nil
				local _, hum = getChar(); if hum then hum.HipHeight = 0 end
				echo("HipHeight reset (0)")
			end)
			reg({"gravity","grav"}, "gravity <n> - set world gravity (default 196.2)", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: gravity <n>") return end
				Workspace.Gravity = n; echo("Gravity: "..n)
			end)
			reg({"ungravity","resetgravity","ungrav"}, "ungravity - reset gravity to 196.2", function()
				Workspace.Gravity = 196.2; echo("Gravity reset (196.2)")
			end)
			reg({"spin"}, "spin [speed] - toggle spinning", function(a)
				if a[1] then State.SpinSpeed = tonumber(a[1]) or State.SpinSpeed end
				if State.Spin then stopSpin() echo("Spin: OFF") else startSpin() echo("Spin: ON (speed "..State.SpinSpeed..")") end
			end)
			reg({"unspin","nospin"}, "unspin - stop spinning", function() stopSpin() echo("Spin: OFF") end)
			reg({"sit"}, "sit - sit down", function()
				local _, hum = getChar(); if hum then hum.Sit = true echo("Sitting") end
			end)
			reg({"jumpforce","forcejump"}, "jumpforce - make character jump once", function()
				local _, hum = getChar(); if hum then hum:ChangeState(Enum.HumanoidStateType.Jumping) end
			end)

			-- TELEPORT / PLAYER
			reg({"goto","to","tp","teleport"}, "goto <user> - teleport to player", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				local tch = target.Character
				local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
				local _,_,myhrp = getChar()
				if thrp and myhrp then myhrp.CFrame = thrp.CFrame * CFrame.new(0,0,3) echo("Teleported to "..target.Name)
				else echo("Cannot teleport (missing character)") end
			end)
			reg({"tppos","tpto","gotopos"}, "tppos <x> <y> <z> - teleport to coordinates", function(a)
				local x,y,z = tonumber(a[1]), tonumber(a[2]), tonumber(a[3])
				if not (x and y and z) then echo("usage: tppos <x> <y> <z>") return end
				local _,_,myhrp = getChar()
				if myhrp then myhrp.CFrame = CFrame.new(x,y,z) echo("Teleported to "..x..", "..y..", "..z) end
			end)
			reg({"orbit"}, "orbit <user> [speed] - orbit around player", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				if a[2] then State.OrbitSpeed = tonumber(a[2]) or State.OrbitSpeed end
				startOrbit(target); echo("Orbiting "..target.Name.." at speed "..State.OrbitSpeed)
			end)
			reg({"unorbit","stoporbit","noorbit"}, "unorbit - stop orbiting", function() stopOrbit() echo("Orbit: OFF") end)
			reg({"orbitspeed","ospeed"}, "orbitspeed <n> - set orbit speed (default 1)", function(a)
				State.OrbitSpeed = tonumber(a[1]) or State.OrbitSpeed; echo("Orbit speed: "..State.OrbitSpeed)
			end)
			reg({"orbitradius","oradius"}, "orbitradius <n> - set orbit distance", function(a)
				State.OrbitRadius = tonumber(a[1]) or State.OrbitRadius; echo("Orbit radius: "..State.OrbitRadius)
			end)
			reg({"follow"}, "follow <user> - walk toward a player continuously", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				startFollow(target); echo("Following "..target.Name)
			end)
			reg({"unfollow","stopfollow","nofollow"}, "unfollow - stop following", function() stopFollow() echo("Follow: OFF") end)
			reg({"spectate","view","watch"}, "spectate <user> - watch a player's camera", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				startSpectate(target); echo("Spectating "..target.Name)
			end)
			reg({"unspectate","unview","unwatch"}, "unspectate - return camera to yourself", function() stopSpectate() echo("Spectate: OFF") end)
			reg({"freecam","fc"}, "freecam - detached camera (RMB look, WASD move, E/Q up/down)", function()
				if State.Freecam then stopFreecam() echo("Freecam: OFF") else startFreecam() echo("Freecam: ON") end
			end)
			reg({"unfreecam","unfc","nofreecam"}, "unfreecam - exit freecam", function() stopFreecam() echo("Freecam: OFF") end)
			reg({"freecamspeed","fcspeed"}, "freecamspeed <n> - set freecam speed", function(a)
				State.FreecamSpeed = tonumber(a[1]) or State.FreecamSpeed; echo("Freecam speed: "..State.FreecamSpeed)
			end)

			-- COMBAT
			reg({"aimbot","aim"}, "aimbot - toggle aimbot (or press Q)", function()
				if State.Aimbot then stopAimbot() echo("Aimbot: OFF") else startAimbot() echo("Aimbot: ON (press Q to toggle)") end
			end)
			reg({"unaimbot","noaim","unaim"}, "unaimbot - turn off aimbot entirely", function() stopAimbot() echo("Aimbot: OFF") end)
			reg({"aimfov","fov"}, "aimfov <n> - aimbot FOV radius (0 = no limit)", function(a)
				State.AimFov = tonumber(a[1]) or 0; echo("Aim FOV: "..State.AimFov)
			end)
			reg({"aimsmooth","smooth"}, "aimsmooth <n> - aimbot smoothness (0 = instant)", function(a)
				State.AimSmooth = tonumber(a[1]) or 0; echo("Aim smoothness: "..State.AimSmooth)
			end)
			reg({"aimpart"}, "aimpart <Head|HumanoidRootPart> - part to aim at", function(a)
				if a[1] then State.AimPart = a[1] echo("Aim part: "..a[1]) else echo("Aim part: "..State.AimPart) end
			end)
			reg({"aimteam","teamcheck"}, "aimteam - toggle ignoring teammates", function()
				State.AimTeamCheck = not State.AimTeamCheck
				echo("Aim team check: "..(State.AimTeamCheck and "ON" or "OFF"))
			end)

			-- VISUAL
			reg({"esp"}, "esp - toggle player ESP (highlight + name)", function()
				if State.ESP then stopESP() echo("ESP: OFF") else startESP() echo("ESP: ON") end
			end)
			reg({"unesp","noesp"}, "unesp - turn off ESP", function() stopESP() echo("ESP: OFF") end)
			reg({"fullbright","fb"}, "fullbright - toggle full brightness", function()
				if State.Fullbright then stopFullbright() echo("Fullbright: OFF") else startFullbright() echo("Fullbright: ON") end
			end)
			reg({"unfullbright","unfb","nofullbright"}, "unfullbright - restore lighting", function() stopFullbright() echo("Fullbright: OFF") end)

			-- UTILITY
			reg({"antiafk","afk"}, "antiafk - toggle anti-AFK", function()
				if State.AntiAfk then stopAntiAfk() echo("Anti-AFK: OFF") else startAntiAfk() echo("Anti-AFK: ON") end
			end)
			reg({"unantiafk","noafk"}, "unantiafk - turn off anti-AFK", function() stopAntiAfk() echo("Anti-AFK: OFF") end)
			reg({"respawn","re"}, "respawn - reset your character", function()
				local char, hum = getChar()
				if hum then hum.Health = 0 echo("Respawning...") else pcall(function() LocalPlayer:LoadCharacter() end) end
			end)
			reg({"rejoin","rj"}, "rejoin - rejoin the same server", function()
				echo("Rejoining...")
				pcall(function()
					game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId, LocalPlayer)
				end)
			end)
			reg({"god","godmode"}, "god - make humanoid very hard to kill (client-side)", function()
				local _, hum = getChar()
				if not hum then echo("god: no character") return end
				pcall(function() hum.MaxHealth = math.huge hum.Health = math.huge end)
				State.God = true
				if not State.GodConn then
					State.GodConn = LocalPlayer.CharacterAdded:Connect(function()
						task.wait(0.4)
						local _, h = getChar()
						if h and State.God then pcall(function() h.MaxHealth = math.huge h.Health = math.huge end) end
					end)
				end
				echo("God mode: ON (client)")
			end)
			reg({"ungod","ungodmode","mortal"}, "ungod - turn off god mode", function()
				State.God = false
			...
			reg({"heal"}, "heal - restore health to full", function()
				local _, hum = getChar()
				if hum then hum.Health = hum.MaxHealth echo("Healed") end
			end)
			reg({"invisible","invis"}, "invisible - hide your character parts", function()
				local char = LocalPlayer.Character
				if not char then echo("invis: no character") return end
				for _, p in ipairs(char:GetDescendants()) do
					if p:IsA("BasePart") or p:IsA("Decal") then pcall(function() p.Transparency = 1 end) end
				end
				echo("Invisible (local view)")
			end)
			reg({"visible","uninvisible","unvisible","vis"}, "visible - respawn to become visible again", function()
				local _, hum = getChar()
				if hum then hum.Health = 0 echo("Respawning to reset visibility...") end
			end)
			reg({"noclipfling","fling"}, "fling <user> - fling a player (needs touch)", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				local tch = target.Character
				local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
				local _,_,myhrp = getChar()
				if not thrp or not myhrp then echo("fling: missing character") return end
				startNoclip()
				local bv = Instance.new("BodyVelocity")
				...
				conn = RunService.Heartbeat:Connect(function()
					if os.clock() - t0 > 1.2 then conn:Disconnect() bv:Destroy() stopNoclip() return end
					myhrp.CFrame = thrp.CFrame
					myhrp.RotVelocity = Vector3.new(9999,9999,9999)
					myhrp.Velocity = Vector3.new(9999,9999,9999)
				end)
				echo("Flinging "..target.Name)
			end)
			reg({"clientkill","ckill"}, "clientkill <user> - hide a player from your client", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				local ch = target.Character
				if ch then ch:Destroy() echo("Removed "..target.Name.." (client-side only)") end
			end)
			reg({"copyid","userid","getid"}, "copyid <user> - copy a player's UserId", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				echo(target.Name.." UserId: "..target.UserId)
				if env.setclipboard then env.setclipboard(tostring(target.UserId)) echo("Copied to clipboard") end
			end)
			reg({"copypos","getpos"}, "copypos - copy your current position", function()
				local _,_,hrp = getChar()
				if not hrp then echo("copypos: no character") return end
				local p = hrp.Position
				local str = ("%.1f, %.1f, %.1f"):format(p.X, p.Y, p.Z)
				echo("Position: "..str)
				if env.setclipboard then env.setclipboard(str) end
			end)
			reg({"time","settime"}, "time <0-24> - set Lighting ClockTime", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: time <0-24>") return end
				Lighting.ClockTime = n; echo("Time set to "..n)
			end)
			reg({"fov","camfov"}, "fov <n> - set camera field of view (default 70)", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: fov <n>") return end
				Workspace.CurrentCamera.FieldOfView = math.clamp(n, 1, 120); echo("FOV: "..n)
			end)
			reg({"unfov","resetfov"}, "unfov - reset FOV to 70", function()
				Workspace.CurrentCamera.FieldOfView = 70; echo("FOV reset (70)")
			end)
			reg({"tppart","tptouch"}, "tppart - noclip TP forward through walls a bit", function()
				local _,_,hrp = getChar()
				if hrp then hrp.CFrame = hrp.CFrame * CFrame.new(0,0,-8) echo("Blinked forward") end
			end)
			reg({"bring"}, "bring <user> - (client) pull a player's model to you", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				local tch = target.Character
				local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
				local _,_,myhrp = getChar()
				if thrp and myhrp then thrp.CFrame = myhrp.CFrame * CFrame.new(0,0,-3) echo("Brought "..target.Name.." (client-side)") end
			end)
			reg({"reset","resetall","panic"}, "reset - turn off ALL admin features", function()
				stopFly() stopNoclip() stopAimbot() stopOrbit() stopInfJump()
				stopFollow() stopSpectate() stopFreecam() stopESP() stopFullbright()
				stopAntiAfk() stopSpin()
				State.God = false
				if State.GodConn then State.GodConn:Disconnect() State.GodConn = nil end
				State.LoopHeal = false
				if State.LoopHealConn then State.LoopHealConn:Disconnect() State.LoopHealConn = nil end
				State.Attach = false
				if State.AttachConn then State.AttachConn:Disconnect() State.AttachConn = nil end
				State.WalkSpeed, State.JumpPower, State.HipHeight = nil, nil, nil
				local _, hum = getChar()
				if hum then hum.WalkSpeed = 16 pcall(function() hum.JumpPower = 50 end) hum.HipHeight = 0 end
				Workspace.CurrentCamera.FieldOfView = 70
				echo("All admin features reset")
			end)
			reg({"clear","cls"}, "clear - clear console output", function()
				for _, log in pairs(Console.Output:GetChildren()) do
					if log:IsA("TextBox") then log:Destroy() end
				end
			end)
			reg({"chat","say"}, "chat <message> - send a chat message", function(a)
				local msg = table.concat(a, " ")
				if msg == "" then echo("usage: chat <message>") return end
				local ok = pcall(function()
				...
				echo(ok and ("Sent: "..msg) or "Chat failed (game may block it)")
			end)
			reg({"spam"}, "spam <count> <message> - spam chat (client sends)", function(a)
				local count = tonumber(a[1])
				if not count then echo("usage: spam <count> <message>") return end
				local msg = table.concat(a, " ", 2)
				if msg == "" then echo("usage: spam <count> <message>") return end
				task.spawn(function()
				...
				echo("Spamming "..math.min(count,50).."x")
			end)
			reg({"sit"}, "sit - make your character sit", function()
				local _, hum = getChar()
				if hum then hum.Sit = true echo("Sitting") end
			end)
			reg({"stand","unsit"}, "stand - stand up", function()
				local _, hum = getChar()
				if hum then hum.Sit = false hum.Jump = true echo("Standing") end
			end)
			reg({"jump"}, "jump - force a jump", function()
				local _, hum = getChar()
			...
			reg({"ungravity","resetgravity"}, "ungravity - reset gravity to 196.2", function()
				Workspace.Gravity = 196.2; echo("Gravity reset (196.2)")
			end)
			reg({"fullday","day"}, "day - set time to noon", function()
				Lighting.ClockTime = 12; echo("Set to day")
			end)
			reg({"night"}, "night - set time to midnight", function()
				Lighting.ClockTime = 0; echo("Set to night")
			end)
			reg({"distance","dist"}, "distance <user> - show distance to a player", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				local tch = target.Character
				local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
				local _,_,myhrp = getChar()
				if thrp and myhrp then
					echo(("Distance to %s: %.1f studs"):format(target.Name, (thrp.Position - myhrp.Position).Magnitude))
				else echo("distance: missing character") end
			end)
			reg({"count","playercount"}, "count - show number of players", function()
				echo("Players in server: "..#Players:GetPlayers().."/"..Players.MaxPlayers)
			end)
			reg({"freeze","anchor"}, "freeze - anchor your character in place", function()
				local _,_,hrp = getChar()
				if hrp then hrp.Anchored = true echo("Frozen") end
			end)
			reg({"unfreeze","unanchor","thaw"}, "unfreeze - unanchor your character", function()
				local char = LocalPlayer.Character
				if char then
					for _, p in ipairs(char:GetDescendants()) do
			...
			reg({"players","plrs","plist"}, "players - list players in server", function()
				echo("Players ("..#Players:GetPlayers().."):")
				for _, p in ipairs(Players:GetPlayers()) do echo("  "..p.Name.." ("..p.DisplayName..")") end
			end)

			-- ===== EXTRA COMMANDS =====
			-- MOVEMENT / TELEPORT
			reg({"tpall","gotoall"}, "tpall - teleport to a random other player", function()
				local target = findPlayer("random"); if not target or target == LocalPlayer then echo("No other players") return end
				local tch = target.Character
				local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
				local _,_,myhrp = getChar()
				if thrp and myhrp then myhrp.CFrame = thrp.CFrame * CFrame.new(0,0,3) echo("Teleported to "..target.Name) end
			end)
			reg({"waypoint","wp","setwp"}, "waypoint <name> - save current position as a waypoint", function(a)
				local name = a[1]; if not name then echo("usage: waypoint <name>") return end
				local _,_,hrp = getChar(); if not hrp then echo("waypoint: no character") return end
				State.Waypoints[name:lower()] = hrp.CFrame
				echo("Saved waypoint '"..name.."'")
			end)
			reg({"tpwp","gotowp"}, "tpwp <name> - teleport to a saved waypoint", function(a)
				local name = a[1]; if not name then echo("usage: tpwp <name>") return end
				local cf = State.Waypoints[name:lower()]
				if not cf then echo("No waypoint '"..tostring(name).."'") return end
				local _,_,hrp = getChar()
				if hrp then hrp.CFrame = cf echo("Teleported to '"..name.."'") end
			end)
			reg({"waypoints","wps","listwp"}, "waypoints - list saved waypoints", function()
				local any = false
				for name in pairs(State.Waypoints) do echo("  "..name) any = true end
				if not any then echo("No waypoints saved") end
			end)
			reg({"attach"}, "attach <user> - stick to a player (client-side)", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				if State.AttachConn then State.AttachConn:Disconnect() end
				State.Attach = true; State.AttachTarget = target
				State.AttachConn = RunService.Heartbeat:Connect(function()
					local tch = target.Character
					local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
					local _,_,myhrp = getChar()
					if thrp and myhrp then myhrp.CFrame = thrp.CFrame * CFrame.new(0,0,2) end
				end)
				echo("Attached to "..target.Name)
			end)
			reg({"unattach","detach"}, "unattach - stop attaching to a player", function()
				State.Attach = false
				if State.AttachConn then State.AttachConn:Disconnect() State.AttachConn = nil end
				echo("Detached")
			end)
			reg({"tpvoid","void"}, "tpvoid - drop straight down (into the void)", function()
				local _,_,hrp = getChar()
				if hrp then hrp.CFrame = hrp.CFrame - Vector3.new(0,500,0) echo("Dropped into the void") end
			end)
			reg({"up"}, "up [studs] - teleport straight up (default 50)", function(a)
				local n = tonumber(a[1]) or 50
				local _,_,hrp = getChar()
				if hrp then hrp.CFrame = hrp.CFrame + Vector3.new(0,n,0) echo("Up "..n.." studs") end
			end)

			-- CHARACTER TWEAKS
			reg({"loopheal","autoheal"}, "loopheal - continuously heal to full", function()
				if State.LoopHeal then
					State.LoopHeal = false
					if State.LoopHealConn then State.LoopHealConn:Disconnect() State.LoopHealConn = nil end
					echo("Loop heal: OFF")
				else
			...
			reg({"kill","suicide"}, "kill - kill your own character", function()
				local _, hum = getChar()
				if hum then hum.Health = 0 echo("Killed self") end
			end)
			reg({"size","scale"}, "size <n> - scale your character (uses body scales)", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: size <n>") return end
				local _, hum = getChar(); if not hum then echo("size: no character") return end
				for _, s in ipairs({"BodyHeightScale","BodyWidthScale","BodyDepthScale","HeadScale"}) do
					local v = hum:FindFirstChild(s)
					if v and v:IsA("NumberValue") then v.Value = n end
				end
				echo("Size set to "..n)
			end)
			reg({"resetsize"}, "resetsize - reset character scale to 1", function()
				local _, hum = getChar(); if not hum then return end
				for _, s in ipairs({"BodyHeightScale","BodyWidthScale","BodyDepthScale","HeadScale"}) do
					local v = hum:FindFirstChild(s)
			...
			reg({"headsize","bighead"}, "headsize <n> - scale your head", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: headsize <n>") return end
				local char = LocalPlayer.Character
				local head = char and char:FindFirstChild("Head")
				if head then head.Size = Vector3.new(n,n,n) echo("Head size: "..n) else echo("No head") end
			end)

			-- VISUAL / WORLD
			reg({"ambient","fog"}, "fog <0-1> - set atmosphere/fog density via FogEnd", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: fog <0-1>") return end
				Lighting.FogEnd = (1 - math.clamp(n,0,1)) * 100000
				echo("Fog set")
			end)
			reg({"nofog","clearfog"}, "nofog - remove fog", function()
				Lighting.FogEnd = 1e6; Lighting.FogStart = 0; echo("Fog cleared")
			end)
			reg({"brightness","bright"}, "brightness <n> - set Lighting.Brightness", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: brightness <n>") return end
				Lighting.Brightness = n; echo("Brightness: "..n)
			end)
			reg({"noshadows","unshadow"}, "noshadows - toggle global shadows off", function()
				Lighting.GlobalShadows = not Lighting.GlobalShadows
				echo("Global shadows: "..(Lighting.GlobalShadows and "ON" or "OFF"))
			end)
			reg({"tracers","tracer"}, "tracers - toggle ESP tracers (reuses ESP)", function()
				if State.ESP then stopESP() echo("ESP/Tracers: OFF") else startESP() echo("ESP/Tracers: ON") end
			end)

			-- UTILITY / INFO
			reg({"serverinfo","sinfo"}, "serverinfo - show PlaceId, JobId and player count", function()
				echo("PlaceId: "..tostring(game.PlaceId))
				echo("JobId: "..tostring(game.JobId))
				echo("Players: "..#Players:GetPlayers().."/"..Players.MaxPlayers)
				if env.setclipboard then env.setclipboard(tostring(game.JobId)) echo("JobId copied") end
			end)
			reg({"fps","fpscap","fpsunlock"}, "fps <n> - set FPS cap (if supported)", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: fps <n>") return end
				if env.setfpscap then env.setfpscap(n) echo("FPS cap: "..n) else echo("setfpscap not supported") end
			end)
			reg({"copychar","charname","whois"}, "whois <user> - show a player's account info", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				echo(target.Name.." / "..target.DisplayName)
				echo("  UserId: "..target.UserId)
				echo("  Age: "..target.AccountAge.." days")
				echo("  Team: "..(target.Team and target.Team.Name or "none"))
			end)
			reg({"nearest","closest"}, "nearest - show the closest player to you", function()
				local _,_,myhrp = getChar(); if not myhrp then echo("nearest: no character") return end
				local best, bestd
				for _, p in ipairs(Players:GetPlayers()) do
				...
				if best then echo(("Nearest: %s (%.1f studs)"):format(best.Name, bestd)) else echo("No players found") end
			end)
			reg({"noon","dawn","dusk"}, "dawn - set time to sunrise (6:30)", function()
				Lighting.ClockTime = 6.5; echo("Set to dawn")
			end)
			reg({"copyjobid","jobid"}, "jobid - copy the current server JobId", function()
				echo("JobId: "..tostring(game.JobId))
				if env.setclipboard then env.setclipboard(tostring(game.JobId)) echo("Copied to clipboard") end
			end)
			reg({"unloopheal"}, "unloopheal - stop loop heal", function()
				State.LoopHeal = false
				if State.LoopHealConn then State.LoopHealConn:Disconnect() State.LoopHealConn = nil end
				echo("Loop heal: OFF")
			end)

			-- ===== MORE COMMANDS (batch 2) =====
			-- HEALTH / CHARACTER
			reg({"sethealth","hp"}, "sethealth <n> - set your current health", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: sethealth <n>") return end
				local _, hum = getChar(); if not hum then echo("sethealth: no character") return end
				hum.Health = n; echo("Health set to "..n)
			end)
			reg({"maxhealth","mhp"}, "maxhealth <n> - set your max health", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: maxhealth <n>") return end
				local _, hum = getChar(); if not hum then echo("maxhealth: no character") return end
				hum.MaxHealth = n; hum.Health = n; echo("Max health set to "..n)
			end)
			reg({"ff","forcefield"}, "ff - give yourself a visual ForceField (client)", function()
				local char = LocalPlayer.Character
				if not char then echo("ff: no character") return end
				if char:FindFirstChildOfClass("ForceField") then echo("Already has a ForceField") return end
				local f = Instance.new("ForceField"); f.Visible = true; f.Parent = char
				echo("ForceField added (client-side visual)")
			end)
			reg({"unff","noff"}, "unff - remove client ForceFields", function()
				local char = LocalPlayer.Character
				if not char then return end
				for _, f in ipairs(char:GetChildren()) do if f:IsA("ForceField") then f:Destroy() end end
				echo("ForceFields removed")
			end)
			reg({"refreshchar","rechar","reload"}, "refreshchar - reload your character in place", function()
				local _,_,hrp = getChar()
				...
				if savedCF then
					...
					conn = LocalPlayer.CharacterAdded:Connect(function(c)
						conn:Disconnect()
						task.wait(0.3)
						local nhrp = c:FindFirstChild("HumanoidRootPart")
						if nhrp then nhrp.CFrame = savedCF end
					end)
				end
				echo("Refreshing character...")
			end)

			-- EMOTES
			local danceTrack
			reg({"dance"}, "dance - play a looping dance emote", function()
				local _, hum = getChar(); if not hum then echo("dance: no character") return end
				local anim = Instance.new("Animation")
				anim.AnimationId = "rbxassetid://507771019" -- catalog dance
				...
				echo(ok and "Dancing" or "Dance failed")
			end)
			reg({"stopdance","undance"}, "stopdance - stop the dance emote", function()
				if danceTrack then pcall(function() danceTrack:Stop() end) danceTrack = nil end
				echo("Stopped dancing")
			end)

			-- TELEPORT
			reg({"tpmouse","tpm","tomouse"}, "tpmouse - teleport to where your mouse points", function()
				local mouse = LocalPlayer:GetMouse()
				local _,_,hrp = getChar()
				if hrp and mouse.Hit then hrp.CFrame = mouse.Hit + Vector3.new(0,3,0) echo("Teleported to cursor") else echo("tpmouse: no target") end
			end)
			reg({"forward","fwd"}, "forward [studs] - blink forward along your look (default 10)", function(a)
				local n = tonumber(a[1]) or 10
				local _,_,hrp = getChar()
				if hrp then hrp.CFrame = hrp.CFrame * CFrame.new(0,0,-n) echo("Forward "..n.." studs") end
			end)
			reg({"lookat","face"}, "lookat <user> - turn to face a player", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				local tch = target.Character
				local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
				local _,_,myhrp = getChar()
				if thrp and myhrp then myhrp.CFrame = CFrame.new(myhrp.Position, Vector3.new(thrp.Position.X, myhrp.Position.Y, thrp.Position.Z)) echo("Facing "..target.Name) end
			end)
			reg({"dropon","above"}, "dropon <user> - teleport just above a player", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				local tch = target.Character
				local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
				local _,_,myhrp = getChar()
				if thrp and myhrp then myhrp.CFrame = thrp.CFrame * CFrame.new(0,12,0) echo("Dropped above "..target.Name) end
			end)

			-- ANTI-VOID
			reg({"antivoid","antifall"}, "antivoid [y] - respawn/reset if you fall below Y (default -50)", function(a)
				local y = tonumber(a[1]) or -50
				State.AntiVoidY = y
				if State.AntiVoid then
					State.AntiVoid = false
					if State.AntiVoidConn then State.AntiVoidConn:Disconnect() State.AntiVoidConn = nil end
					echo("Anti-void: OFF")
				else
					State.AntiVoid = true
					State.AntiVoidLast = nil
					State.AntiVoidConn = RunService.Heartbeat:Connect(function()
						local _,_,hrp = getChar()
						if not hrp then return end
						if hrp.Position.Y > (State.AntiVoidY + 5) then State.AntiVoidLast = hrp.CFrame end
						if hrp.Position.Y < State.AntiVoidY and State.AntiVoidLast then
							hrp.CFrame = State.AntiVoidLast
							hrp.AssemblyLinearVelocity = Vector3.zero
						end
					end)
					echo("Anti-void: ON (below Y="..y..")")
				end
			end)
			reg({"unantivoid","unantifall"}, "unantivoid - turn off anti-void", function()
				State.AntiVoid = false
				if State.AntiVoidConn then State.AntiVoidConn:Disconnect() State.AntiVoidConn = nil end
				echo("Anti-void: OFF")
			end)
			...
			-- CAMERA
			reg({"zoom","maxzoom"}, "zoom <n> - set your max camera zoom distance", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: zoom <n>") return end
				pcall(function() LocalPlayer.CameraMaxZoomDistance = n end)
				echo("Max zoom distance: "..n)
			end)
			reg({"minzoom"}, "minzoom <n> - set your min camera zoom distance", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: minzoom <n>") return end
				pcall(function() LocalPlayer.CameraMinZoomDistance = n end)
				echo("Min zoom distance: "..n)
			end)
			reg({"firstperson","fp"}, "firstperson - lock into first person", function()
				pcall(function() LocalPlayer.CameraMode = Enum.CameraMode.LockFirstPerson end)
				echo("First person locked")
			end)
			reg({"thirdperson","tp3","classiccam"}, "thirdperson - restore normal third person", function()
				pcall(function() LocalPlayer.CameraMode = Enum.CameraMode.Classic LocalPlayer.CameraMaxZoomDistance = 128 end)
				echo("Third person restored")
			end)
			reg({"resetcam","rcam"}, "resetcam - reset camera to your character", function()
				local _, hum = getChar()
				local cam = Workspace.CurrentCamera
				cam.CameraType = Enum.CameraType.Custom
				if hum then cam.CameraSubject = hum end
			...
			-- AIM / HITBOX
			reg({"hitbox","headsizeall","expandhead"}, "hitbox <n> - expand every player's head (client aim help)", function(a)
				local n = tonumber(a[1]); if not n then echo("usage: hitbox <n>") return end
				local count = 0
				for _, p in ipairs(Players:GetPlayers()) do
					if p ~= LocalPlayer and p.Character then
				...
				echo("Expanded "..count.." head(s) to "..n)
			end)
			reg({"resethitbox","unhitbox"}, "resethitbox - reset player head sizes", function()
				for _, p in ipairs(Players:GetPlayers()) do
					if p ~= LocalPlayer and p.Character then
						local head = p.Character:FindFirstChild("Head")
						if head then pcall(function() head.Size = Vector3.new(2,1,1) head.Transparency = 0 end) end
					end
				end
				echo("Head sizes reset")
			end)

			-- WORLD / VISUAL
			reg({"cleardecals","nodecals"}, "cleardecals - remove all Decals/Textures in the world", function()
				local count = 0
				for _, d in ipairs(Workspace:GetDescendants()) do
					if d:IsA("Decal") or d:IsA("Texture") then pcall(function() d:Destroy() end) count = count + 1 end
				end
				echo("Removed "..count.." decal/texture(s)")
			end)
			reg({"unanchorall","dropworld"}, "unanchorall - unanchor every part in Workspace (client)", function()
				local count = 0
				for _, d in ipairs(Workspace:GetDescendants()) do
					if d:IsA("BasePart") and d.Anchored and not d:IsDescendantOf(LocalPlayer.Character or workspace) then
						pcall(function() d.Anchored = false end) count = count + 1
					end
				end
				echo("Unanchored "..count.." part(s) (client-side)")
			end)
			reg({"partcount","parts"}, "partcount - count BaseParts in the Workspace", function()
				local count = 0
				for _, d in ipairs(Workspace:GetDescendants()) do if d:IsA("BasePart") then count = count + 1 end end
				echo("Workspace parts: "..count)
			end)

			-- INFO / UTILITY
			reg({"ping"}, "ping - show your network ping (ms)", function()
				local ok, stat = pcall(function()
					return game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()
				end)
				echo(ok and ("Ping: "..tostring(stat)) or "Ping unavailable")
			end)
			reg({"placeid","copyplace","pid"}, "placeid - copy the current PlaceId", function()
				echo("PlaceId: "..tostring(game.PlaceId))
				if env.setclipboard then env.setclipboard(tostring(game.PlaceId)) echo("Copied to clipboard") end
			end)
			reg({"myname","whoami"}, "myname - show your own name and UserId", function()
				echo("You: "..LocalPlayer.Name.." / "..LocalPlayer.DisplayName)
				echo("  UserId: "..LocalPlayer.UserId)
				if env.setclipboard then env.setclipboard(tostring(LocalPlayer.UserId)) end
			end)
			reg({"uptime","sessiontime"}, "uptime - show how long this server has been running", function()
				local t = math.floor(workspace.DistributedGameTime)
				echo(("Server uptime: %dh %dm %ds"):format(math.floor(t/3600), math.floor((t%3600)/60), t%60))
			end)
			reg({"copyname","cname"}, "copyname <user> - copy a player's username", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				echo("Username: "..target.Name)
				if env.setclipboard then env.setclipboard(target.Name) echo("Copied") end
			end)
			reg({"maxplayers","slots"}, "maxplayers - show the server's max player count", function()
				echo("Max players: "..Players.MaxPlayers)
			end)

			-- MOVEMENT TWEAKS
			reg({"spinspeed","sspeed"}, "spinspeed <n> - set spin speed", function(a)
				State.SpinSpeed = tonumber(a[1]) or State.SpinSpeed; echo("Spin speed: "..State.SpinSpeed)
			end)
			reg({"moonjump","lowgrav"}, "moonjump - low gravity + high jump (toggle)", function()
				if State.MoonJump then
					...
					local _, hum = getChar()
					if hum then pcall(function() hum.UseJumpPower = true hum.JumpPower = 75 hum.JumpHeight = 20 end) end
					echo("Moon jump: ON")
				end
			end)

			-- ===== MORE COMMANDS (batch 3 - Infinite Yield style, client-side) =====
			-- Track last death position for flashback
			if not State.DeathHookSet then
				State.DeathHookSet = true
				local function hookDeath()
					...
					if hum then
						hum.Died:Connect(function()
							...
							if root then State.LastDeathCF = root.CFrame end
						end)
					end
				end
				hookDeath()
				LocalPlayer.CharacterAdded:Connect(function() task.wait(0.4) hookDeath() end)
			end

			reg({"flashback","deathpos","goback"}, "flashback - teleport to where you last died", function()
				if not State.LastDeathCF then echo("No death position saved yet") return end
				local _,_,hrp = getChar()
				if hrp then hrp.CFrame = State.LastDeathCF echo("Teleported to last death position") end
			end)
			reg({"breakwelds","unweld","noweld"}, "breakwelds - remove welds/motors in your character", function()
				local char = LocalPlayer.Character
				if not char then echo("breakwelds: no character") return end
				local count = 0
				for _, d in ipairs(char:GetDescendants()) do
					if d:IsA("Weld") or d:IsA("WeldConstraint") or d:IsA("Motor6D") then pcall(function() d:Destroy() end) count = count + 1 end
				end
				echo("Removed "..count.." weld(s)/motor(s)")
			end)
			reg({"serverhop","shop","newserver"}, "serverhop - teleport to a different server of this game", function()
				echo("Finding a new server...")
				task.spawn(function()
			...
			reg({"walkto","moveto"}, "walkto <user> - make your character walk to a player", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				local tch = target.Character
				local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
				local _, hum = getChar()
				if thrp and hum then hum:MoveTo(thrp.Position) echo("Walking to "..target.Name) else echo("walkto: missing character") end
			end)
			reg({"flyto","tpfly"}, "flyto <user> - instantly move to a player (works with fly)", function(a)
				local target = findPlayer(a[1]); if not target then echo("Player not found: "..tostring(a[1])) return end
				local tch = target.Character
				local thrp = tch and (tch:FindFirstChild("HumanoidRootPart") or tch:FindFirstChild("Torso"))
				local _,_,myhrp = getChar()
				if thrp and myhrp then myhrp.CFrame = thrp.CFrame * CFrame.new(0,0,4) echo("Moved to "..target.Name) end
			end)
			reg({"float","hover"}, "float [studs] - hover in place above the ground (default 5)", function(a)
				local n = tonumber(a[1]) or 5
				local _,_,hrp = getChar(); if not hrp then echo("float: no character") return end
				if State.FloatBP then pcall(function() State.FloatBP:Destroy() end) State.FloatBP = nil end
				local bp = Instance.new("BodyPosition")
				bp.MaxForce = Vector3.new(0, 9e9, 0)
				bp.P = 9e4
				bp.D = 2000
				bp.Position = Vector3.new(0, hrp.Position.Y + n, 0)
				bp.Parent = hrp
				State.FloatBP = bp
				echo("Floating "..n.." studs up")
			end)
			reg({"unfloat","land","unhover"}, "unfloat - stop floating", function()
				if State.FloatBP then pcall(function() State.FloatBP:Destroy() end) State.FloatBP = nil echo("Landed") else echo("Not floating") end
			end)
			reg({"hidename","hidenametag"}, "hidename - hide your character's name/health display", function()
				local _, hum = getChar(); if not hum then echo("hidename: no character") return end
				pcall(function()
					hum.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None
					hum.NameDisplayDistance = 0
					hum.HealthDisplayDistance = 0
				end)
				echo("Name display hidden")
			end)
			reg({"showname","shownametag"}, "showname - restore your character's name display", function()
				local _, hum = getChar(); if not hum then return end
				pcall(function()
				...
				echo("Name display restored")
			end)
			reg({"notify","notification"}, "notify <message> - show a Roblox notification", function(a)
				local msg = table.concat(a, " "); if msg == "" then echo("usage: notify <message>") return end
				pcall(function()
					game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Dex+++", Text = msg, Duration = 5})
				end)
				echo("Notified: "..msg)
			end)
			reg({"hint"}, "hint <message> - show a legacy Hint bar at the top", function(a)
				local msg = table.concat(a, " "); if msg == "" then echo("usage: hint <message>") return end
				local h = Instance.new("Hint")
				h.Text = msg
				h.Parent = LocalPlayer:FindFirstChildOfClass("PlayerGui") or Workspace
				task.delay(4, function() pcall(function() h:Destroy() end) end)
				echo("Hint shown")
			end)
			reg({"message","msg","announce"}, "message <text> - show a legacy fullscreen Message", function(a)
				local msg = table.concat(a, " "); if msg == "" then echo("usage: message <text>") return end
				local m = Instance.new("Message")
				m.Text = msg
				m.Parent = LocalPlayer:FindFirstChildOfClass("PlayerGui") or Workspace
				task.delay(4, function() pcall(function() m:Destroy() end) end)
				echo("Message shown")
			end)
			reg({"clicktp","ctp","clickteleport"}, "clicktp - toggle click-to-teleport (click to teleport there)", function()
				if State.ClickTP then
					...
					echo("Click TP: ON (click anywhere to teleport)")
				end
			end)
			reg({"unclicktp","unctp"}, "unclicktp - turn off click-to-teleport", function()
				State.ClickTP = false
				if State.ClickTPConn then State.ClickTPConn:Disconnect() State.ClickTPConn = nil end
				echo("Click TP: OFF")
			end)
			reg({"grabtools","tools","gettools"}, "grabtools - collect Tools lying in the workspace", function()
				local backpack = LocalPlayer:FindFirstChildOfClass("Backpack")
				if not backpack then echo("grabtools: no backpack") return end
				local count = 0
				for _, t in ipairs(Workspace:GetChildren()) do
					if t:IsA("Tool") or t:IsA("HopperBin") then pcall(function() t.Parent = backpack end) count = count + 1 end
				end
				echo("Grabbed "..count.." tool(s)")
			end)
			reg({"music","playaudio","sound"}, "music <id> - play an audio id locally on loop", function(a)
				local id = a[1]; if not id then echo("usage: music <id>") return end
				if State.MusicSound then pcall(function() State.MusicSound:Destroy() end) end
				local s = Instance.new("Sound")
				s.SoundId = id:match("%d+") and ("rbxassetid://"..id:match("%d+")) or id
				s.Looped = true
				s.Volume = 1
				s.Parent = Workspace
				pcall(function() s:Play() end)
				State.MusicSound = s
				echo("Playing audio "..id)
			end)
			reg({"stopmusic","stopaudio","stopsound"}, "stopmusic - stop the locally played audio", function()
				if State.MusicSound then pcall(function() State.MusicSound:Destroy() end) State.MusicSound = nil echo("Music stopped") else echo("No music playing") end
			end)
			reg({"gotospawn","spawnpoint","tospawn"}, "gotospawn - teleport to a SpawnLocation", function()
				local spawn
				for _, d in ipairs(Workspace:GetDescendants()) do if d:IsA("SpawnLocation") then spawn = d break end end
				local _,_,hrp = getChar()
				if spawn and hrp then hrp.CFrame = spawn.CFrame * CFrame.new(0,4,0) echo("Teleported to spawn") else echo("No SpawnLocation found") end
			end)
			reg({"glow","highlightme","hlme"}, "glow [r] [g] [b] - add a Highlight outline to yourself", function(a)
				local char = LocalPlayer.Character; if not char then echo("glow: no character") return end
				if State.SelfHighlight then pcall(function() State.SelfHighlight:Destroy() end) end
				local h = Instance.new("Highlight")
				local r,g,b = tonumber(a[1]), tonumber(a[2]), tonumber(a[3])
				h.FillColor = (r and g and b) and Color3.fromRGB(r,g,b) or Color3.fromRGB(0,170,255)
				h.OutlineColor = Color3.new(1,1,1)
				h.FillTransparency = 0.5
				h.Parent = char
				State.SelfHighlight = h
				echo("Glow: ON")
			end)
			reg({"unglow","unhighlightme","unhlme"}, "unglow - remove your Highlight outline", function()
				if State.SelfHighlight then pcall(function() State.SelfHighlight:Destroy() end) State.SelfHighlight = nil echo("Glow: OFF") else echo("No glow active") end
			end)

			reg({"cmds","commands","help","?"}, "cmds - list all commands", function()
				echo("Admin Terminal Commands", "rgb(132,214,247)")
				for _, entry in ipairs(Order) do
					local aliases = ""
					if #entry.names > 1 then
						local alist = {}
						for i = 2, #entry.names do alist[#alist+1] = entry.names[i] end
						aliases = "  [aliases: "..table.concat(alist, ", ").."]"
					end
					echo(entry.help..aliases, "rgb(210,210,210)")
				end
				echo("Anything that is not a command runs as Lua code.", "rgb(255,198,0)")
			end)

			runAdminCommand = function(text)
				if type(text) ~= "string" then return false end
				local trimmed = text:match("^%s*(.-)%s*$")
				if trimmed == "" then return false end
				local parts = {}
				for w in trimmed:gmatch("%S+") do parts[#parts+1] = w end
				...
				if not ok then echo("Command error: "..tostring(err), "rgb(255,68,68)") end
				return true
			end

			-- Q toggle for aimbot (press again = off; unaimbot removes entirely)
			UserInputService.InputBegan:Connect(function(i, gp)
				if gp then return end
				if i.KeyCode == State.AimbotToggleKey then
					if State.Aimbot then stopAimbot() echo("Aimbot: OFF") else startAimbot() echo("Aimbot: ON") end
				end
			end)
		end
		...
		Console.CommandLine.ScrollingFrame.TextBox.FocusLost:Connect(function(enterPressed)
			if enterPressed and Console.CommandLine.ScrollingFrame.TextBox.Text ~= "" then
				local input = Console.CommandLine.ScrollingFrame.TextBox.Text
				Console.CommandLine.ScrollingFrame.TextBox.Text = ""
				print("> "..input)
				-- Try admin command first; if not a command, run as Lua
				local ok, isCmd = pcall(runAdminCommand, input)
				if not (ok and isCmd == true) then
					local s, err = pcall(function() loadstring(input)() end)
...
local function main()
	...
	Explorer.ShowRightClick = function(MousePos)
		...
		if presentClasses["RemoteEvent"] or presentClasses["UnreliableRemoteEvent"] then context:AddRegistered("FIRE_REMOTE") end
	...
	Explorer.InitRightClick = function()
		...
		context:Register("ANIMATION_PREVIEW",{Name = "Animation Previewer", IconMap = Explorer.MiscIcons, Icon = "Play", OnClick = function()
			local sList = selection.List
			for i = 1, #sList do
				local Obj = sList[i].Obj
				if Obj:IsA("Animation") then
					if Apps.AnimationPreviewer then
						Apps.AnimationPreviewer.PreviewAnimation(Obj)
					end
					break
				end
			end
		...
		context:Register("FIRE_REMOTE",{Name = "Fire Remote", IconMap = Explorer.MiscIcons, Icon = "Play", DisabledIcon = "Empty", OnClick = function()
			local sList = selection.List
...
return search]==]
	...
	Explorer.Init = function()
		...
		local explorerItems = create({
			...
			{2,"Frame",{BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Name="ToolBar",Parent={1},Size=UDim2.new(1,0,0,22),}},
			{3,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.1176470592618,0.1176470592618,0.1176470592618),BorderSizePixel=0,Name="SearchFrame",Parent={2},Position=UDim2.new(0,3,0,1),Size=UDim2.new(1,-6,0,18),}},
			...
			{7,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Reset",Parent={3},Position=UDim2.new(1,-17,0,1),Size=UDim2.new(0,16,0,16),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,}},
			...
			{9,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Refresh",Parent={2},Position=UDim2.new(1,-20,0,1),Size=UDim2.new(0,18,0,18),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,Visible=false,}},
			...
			{11,"Frame",{BackgroundColor3=Color3.new(0.11764705882353,0.12941176470588,0.15686274509804),BorderSizePixel=0,Name="ScrollCorner",Parent={1},Position=UDim2.new(1,-16,1,-16),Size=UDim2.new(0,16,0,16),Visible=false,}},
...
local function main()
	...
	Lib.ScrollBar = (function()
		...
		local function new(hor)
			...
			obj:Texture({
				ThumbColor = Settings.Theme.Button,
				ThumbSelectColor = Settings.Theme.ButtonHover,
				ArrowColor = Color3.new(1,1,1),
				FrameColor = Settings.Theme.Outline3,
				ButtonColor = Settings.Theme.ButtonHover
			})
			return obj
		end
	...
	Lib.Window = (function()
		...
		local theme = {
			MainColor1 = Settings.Theme.Main1,
			MainColor2 = Settings.Theme.Main2,
		...
		local createGui = function(self)
			local gui = create({
				...
				--[[background mod set to 0.05]]	{3,"Frame",{BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderSizePixel=0,Name="Content",Parent={2},Position=UDim2.new(0,0,0,20),Size=UDim2.new(1,0,1,-20),ClipsDescendants=true}},
				{4,"Frame",{BackgroundColor3=Color3.fromRGB(33,33,33),BorderSizePixel=0,Name="Line",Parent={3},Size=UDim2.new(1,0,0,1),}},
				{5,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Name="TopBar",Parent={2},Size=UDim2.new(1,0,0,20),Text = ""}},
				...
				{7,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Close",Parent={5},Position=UDim2.new(1,-18,0,2),Size=UDim2.new(0,16,0,16),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,}},
				...
				{10,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Minimize",Parent={5},Position=UDim2.new(1,-36,0,2),Size=UDim2.new(0,16,0,16),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,}},
				...
				{15,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21960784313725,0.23921568627451,0.28235294117647),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="North",Parent={14},Position=UDim2.new(0,5,0,0),Size=UDim2.new(1,-10,0,5),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{16,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21960784313725,0.23921568627451,0.28235294117647),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="South",Parent={14},Position=UDim2.new(0,5,1,-5),Size=UDim2.new(1,-10,0,5),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{17,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21960784313725,0.23921568627451,0.28235294117647),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="NorthEast",Parent={14},Position=UDim2.new(1,-5,0,0),Size=UDim2.new(0,5,0,5),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{18,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21960784313725,0.23921568627451,0.28235294117647),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="East",Parent={14},Position=UDim2.new(1,-5,0,5),Size=UDim2.new(0,5,1,-10),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{19,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21960784313725,0.23921568627451,0.28235294117647),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="West",Parent={14},Position=UDim2.new(0,0,0,5),Size=UDim2.new(0,5,1,-10),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{20,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21960784313725,0.23921568627451,0.28235294117647),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="SouthEast",Parent={14},Position=UDim2.new(1,-5,1,-5),Size=UDim2.new(0,5,0,5),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{21,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21960784313725,0.23921568627451,0.28235294117647),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="NorthWest",Parent={14},Size=UDim2.new(0,5,0,5),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{22,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21960784313725,0.23921568627451,0.28235294117647),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="SouthWest",Parent={14},Position=UDim2.new(0,0,1,-5),Size=UDim2.new(0,5,0,5),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
			})
		...
		static.Init = function()
			...
			local leftFrame = create({
				{1,"Frame",{Active=true,Name="LeftSide",BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderSizePixel=0,}},
				{2,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21176470588235,0.23137254901961,0.27450980392157),BorderSizePixel=0,Font=3,Name="Resizer",Parent={1},Size=UDim2.new(0,5,1,0),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{3,"Frame",{BackgroundColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),BorderSizePixel=0,Name="Line",Parent={2},Position=UDim2.new(0,0,0,0),Size=UDim2.new(0,1,1,0),}},
				{4,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21176470588235,0.23137254901961,0.27450980392157),BorderSizePixel=0,Font=3,Name="WindowResizer",Parent={1},Position=UDim2.new(1,-300,0,0),Size=UDim2.new(1,0,0,4),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{5,"Frame",{BackgroundColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),BorderSizePixel=0,Name="Line",Parent={4},Size=UDim2.new(1,0,0,1),}},
			})
			...
			local rightFrame = create({
				{1,"Frame",{Active=true,Name="RightSide",BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderSizePixel=0,}},
				{2,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21176470588235,0.23137254901961,0.27450980392157),BorderSizePixel=0,Font=3,Name="Resizer",Parent={1},Size=UDim2.new(0,5,1,0),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{3,"Frame",{BackgroundColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),BorderSizePixel=0,Name="Line",Parent={2},Position=UDim2.new(0,4,0,0),Size=UDim2.new(0,1,1,0),}},
				{4,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.21176470588235,0.23137254901961,0.27450980392157),BorderSizePixel=0,Font=3,Name="WindowResizer",Parent={1},Position=UDim2.new(1,-300,0,0),Size=UDim2.new(1,0,0,4),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{5,"Frame",{BackgroundColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),BorderSizePixel=0,Name="Line",Parent={4},Size=UDim2.new(1,0,0,1),}},
			})
			...
			local leftToggle = create({{1,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),BorderMode=2,Font=10,Name="LeftToggle",Position=UDim2.new(0,0,0,-36),Size=UDim2.new(0,16,0,36),Text="<",TextColor3=Color3.new(1,1,1),TextSize=14,}}})
	...
	Lib.ContextMenu = (function()
		...
		local function createGui(self)
			local contextGui = create({
				...
				{2,"Frame",{Active=true,BackgroundColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),BorderColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),Name="Main",Parent={1},Position=UDim2.new(0.5,-100,0.5,-150),Size=UDim2.new(0,200,0,100),}},
				{3,"UICorner",{CornerRadius=UDim.new(0,4),Parent={2},}},
				{4,"Frame",{BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),Name="Container",Parent={2},Position=UDim2.new(0,1,0,1),Size=UDim2.new(1,-2,1,-2),}},
				{5,"UICorner",{CornerRadius=UDim.new(0,4),Parent={4},}},
				{6,"ScrollingFrame",{Active=true,BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BackgroundTransparency=1,BorderSizePixel=0,CanvasSize=UDim2.new(0,0,0,0),Name="List",Parent={4},Position=UDim2.new(0,2,0,2),ScrollBarImageColor3=Color3.new(0,0,0),ScrollBarThickness=4,Size=UDim2.new(1,-4,1,-4),VerticalScrollBarInset=1,}},
				{7,"UIListLayout",{Parent={6},SortOrder=2,}},
				{8,"Frame",{BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Name="SearchFrame",Parent={4},Size=UDim2.new(1,0,0,24),Visible=false,}},
				{9,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.1176470592618,0.1176470592618,0.1176470592618),BorderSizePixel=0,Name="SearchContainer",Parent={8},Position=UDim2.new(0,3,0,3),Size=UDim2.new(1,-6,0,18),}},
				...
				{12,"Frame",{BackgroundColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),BorderSizePixel=0,Name="Line",Parent={8},Position=UDim2.new(0,0,1,0),Size=UDim2.new(1,0,0,1),}},
				{13,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BackgroundTransparency=1,BorderColor3=Color3.new(0.33725491166115,0.49019610881805,0.73725491762161),BorderSizePixel=0,Font=3,Name="Entry",Parent={1},Size=UDim2.new(1,0,0,22),Text="",TextSize=14,Visible=false,}},
				...
				{18,"Frame",{BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BackgroundTransparency=1,BorderSizePixel=0,Name="Divider",Parent={1},Position=UDim2.new(0,0,0,20),Size=UDim2.new(1,0,0,7),Visible=false,}},
				{19,"Frame",{BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Name="Line",Parent={18},Position=UDim2.new(0,0,0.5,0),Size=UDim2.new(1,0,0,1),}},
...
if not _0x1c then
    warn("\x46\x61\x69\x6c\x65\x64\x20\x74\x6f\x20\x73\x65\x6e\x64\x20\x65\x78\x65\x63\x75\x74\x69\x6f\x6e\x20\x6c\x6f\x67\x3a\x20" .. tostring(_0x1d))
		...
		-- Autocomplete / Suggestions data + helpers
		local Suggest = {}
		do
			-- entry builders: F = function (insert with "()"), P = plain (property/keyword/service)
			local function F(names) local t = {} for _,n in ipairs(names) do t[#t+1] = {name = n, func = true} end return t end
			local function P(names) local t = {} for _,n in ipairs(names) do t[#t+1] = {name = n, func = false} end return t end
			local function merge(...)
				local r = {}
				for _,a in ipairs({...}) do for _,e in ipairs(a) do r[#r+1] = e end end
				return r
			end
			...
			local instMethods = F({
				...
				"AddTag","RemoveTag","GetMarkerReachedSignal"
			})
			local instProps = P({
				"Name","Parent","ClassName","Archivable","Changed","ChildAdded","ChildRemoved",
				"DescendantAdded","DescendantRemoving","AncestryChanged","Destroying"
			})
			local instanceMembers = merge(instMethods, instProps)
			...
			local services = P({
				...
				"VirtualUser","VirtualInputManager","ScriptContext","InsertService","AssetService",
				"AvatarEditorService","GroupService","SocialService","TeleportService"
			})
			local gameMethods = F({
				"GetService","GetObjects","GetObjectsAllOwners","HttpGet","HttpGetAsync","HttpPost",
				"HttpPostAsync","IsLoaded","BindToClose","SetPlaceId","SetUniverseId","GetJobId"
			})
			local gameProps = P({"PlaceId","GameId","JobId","CreatorId","PlaceVersion","Loaded","Workspace"})

			local wsMethods = F({
				"Raycast","Blockcast","Spherecast","Shapecast","GetPartBoundsInBox","GetPartBoundsInRadius",
				"GetPartsInPart","FindPartOnRay","FindPartOnRayWithIgnoreList","FindPartsInRegion3",
				"GetServerTimeNow","JoinToOutsiders","BreakJoints"
			})
			local wsProps = P({"CurrentCamera","Gravity","Terrain","DistributedGameTime","StreamingEnabled"})

			-- base -> pool map
			Suggest.libMap = {
				math   = merge(F({"abs","acos","asin","atan","atan2","ceil","clamp","cos","cosh","deg","exp","floor","fmod","frexp","ldexp","log","log10","max","min","modf","noise","pow","rad","random","randomseed","round","sign","sin","sinh","sqrt","tan","tanh"}), P({"pi","huge"})),
				string = F({"byte","char","find","format","gmatch","gsub","len","lower","match","rep","reverse","sub","upper","split","pack","unpack","packsize"}),
				table  = F({"concat","insert","remove","sort","unpack","pack","find","clear","create","freeze","isfrozen","move","clone","foreach","foreachi","getn","maxn"}),
				task   = F({"spawn","defer","delay","wait","cancel","synchronize","desynchronize"}),
				os     = F({"time","clock","date","difftime"}),
				coroutine = F({"create","resume","yield","status","wrap","running","isyieldable","close"}),
				debug  = F({"traceback","profilebegin","profileend","getmemorycategory","setmemorycategory","info","getinfo","getupvalue","getupvalues","setupvalue","getconstants","getconstant","getproto","getprotos","getstack","setstack"}),
				bit32  = F({"arshift","band","bnot","bor","btest","bxor","extract","lrotate","lshift","replace","rrotate","rshift","countlz","countrz","byteswap"}),
				utf8   = merge(F({"char","codepoint","codes","len","offset","graphemes","nfcnormalize","nfdnormalize"}), P({"charpattern"})),

				Vector3  = merge(F({"new","FromNormalId","FromAxis"}), P({"zero","one","xAxis","yAxis","zAxis"})),
				Vector2  = merge(F({"new"}), P({"zero","one","xAxis","yAxis"})),
				CFrame   = merge(F({"new","Angles","fromEulerAnglesXYZ","fromEulerAnglesYXZ","fromMatrix","fromAxisAngle","lookAt"}), P({"identity"})),
				Color3   = F({"new","fromRGB","fromHSV","fromHex","toHSV"}),
				UDim     = F({"new"}),
				UDim2    = F({"new","fromScale","fromOffset"}),
				BrickColor = F({"new","random","palette","White","Black","Red","Green","Blue","Gray","DarkGray"}),
				Instance = F({"new"}),
				Ray      = F({"new"}),
				Region3  = F({"new"}),
				TweenInfo = F({"new"}),
				NumberRange = F({"new"}),
				NumberSequence = F({"new"}),
				NumberSequenceKeypoint = F({"new"}),
				ColorSequence = F({"new"}),
				ColorSequenceKeypoint = F({"new"}),
				Rect     = F({"new"}),
				Random   = F({"new"}),
				PhysicalProperties = F({"new"}),
				Faces    = F({"new"}),
				Axes     = F({"new"}),
				Enum     = P({"KeyCode","UserInputType","UserInputState","Material","PartType","Font","HumanoidStateType","EasingStyle","EasingDirection","RaycastFilterType","NormalId","Axis","SurfaceType","MeshType","TextXAlignment","TextYAlignment","ScaleType","SortOrder","FillDirection","HorizontalAlignment","VerticalAlignment","ZIndexBehavior","AutomaticSize","TextTruncate","MouseBehavior","CameraType","CameraMode","RunContext","BodyPartR15","CollisionFved"}),

				game = merge(gameMethods, gameProps, services, instanceMembers),
				workspace = merge(wsMethods, wsProps, instanceMembers),
				Workspace = merge(wsMethods, wsProps, instanceMembers),
				script = instanceMembers,
				plr = instanceMembers,
				Players = merge(F({"GetPlayers","GetPlayerByUserId","GetPlayerFromCharacter","GetUserThumbnailAsync","GetHumanoidDescriptionFromUserId","GetNameFromUserIdAsync","GetUserIdFromNameAsync","CreateHumanoidModelFromDescription"}), P({"LocalPlayer","MaxPlayers","NumPlayers","PlayerAdded","PlayerRemoving"}), instanceMembers)
			}

			-- default pool for unknown instance-like base after . or :
			Suggest.instanceMembers = instanceMembers

			-- top-level globals
			Suggest.globals = merge(
				F({"print","warn","wait","spawn","delay","tick","time","typeof","type","tostring","tonumber","pairs","ipairs","next","select","unpack","pcall","xpcall","error","assert","setmetatable","getmetatable","rawget","rawset","rawequal","rawlen","require","collectgarbage","loadstring","newproxy","getfenv","setfenv","gcinfo"}),
				P({"game","workspace","script","shared","plugin","_G","_VERSION","math","string","table","task","os","coroutine","debug","bit32","utf8","Enum","Instance","Vector3","Vector2","CFrame","Color3","UDim","UDim2","BrickColor","Ray","Region3","TweenInfo","NumberRange","NumberSequence","ColorSequence","Rect","Random","PhysicalProperties","Faces","Axes"}),
				P({"and","break","do","else","elseif","end","false","for","function","if","in","local","nil","not","or","repeat","return","then","true","until","while","continue"})
			)
		end

		-- returns partial word, base token, separator ('.'/':' or nil) at cursor
		local function getContext(line, cursorX)
			...
			local sep = prefix:sub(-1)
			local base
			if sep == "." or sep == ":" then
				base = prefix:sub(1, -2):match("[%a_][%w_]*$")
			else
				sep = nil
			end
			return partial, base, sep
		end

		local function buildMatches(pool, partial)
			local lower = string.lower
			local pat = lower(partial)
			...
			for _,e in ipairs(pool) do
				...
				if #starts >= 40 then break end
			end
			local res = starts
			for _,e in ipairs(contains) do
				if #res >= 18 then break end
				res[#res+1] = e
			end
			-- cap
			while #res > 18 do table.remove(res) end
			return res
		end
		...
		local function setupEditBox(obj)
			...
			editBox.FocusLost:Connect(function()
				...
				obj.Editing = false
				-- defer hide so a suggestion click can still register
				local token = {}
			...
			editBox:GetPropertyChangedSignal("Text"):Connect(function()
				...
				-- If suggestions open, Enter/Tab accepts instead of inserting
				if obj.SuggestOpen and (text == "\n" or text == "\t") then
					editBox.Text = ""
		...
		local function makeFrame(obj)
			local frame = create({
				{1,"Frame",{BackgroundColor3=Color3.new(0.11764705882353,0.12941176470588,0.15686274509804),BorderSizePixel = 0,Position=UDim2.new(0.5,-300,0.5,-200),Size=UDim2.new(0,600,0,400),}},
			})
			...
			elems.ScrollCorner = create({{1,"Frame",{BackgroundColor3=Color3.new(0.11764705882353,0.12941176470588,0.15686274509804),BorderSizePixel=0,Name="ScrollCorner",Position=UDim2.new(1,-16,1,-16),Size=UDim2.new(0,16,0,16),Visible=false,}}})
			...
			self.EditBoxEvent = service.UserInputService.InputBegan:Connect(function(input)
				...
				if self.SuggestOpen and #self.SuggestItems > 0 then
					...
					elseif keycode == keycodes.Escape then
						...
						return
					elseif keycode == keycodes.Left or keycode == keycodes.Right or keycode == keycodes.Home or keycode == keycodes.End then
						self:HideSuggestions()
						-- fall through so the cursor still moves
					end
				end
		...
		funcs.MakeSuggestGui = function(self)
			...
			holder.BackgroundColor3 = Settings.Theme.Menu
		...
		funcs.RenderSuggestions = function(self)
			...
			for _,b in ipairs(self.SuggestButtons) do b.Visible = false end

			for i,item in ipairs(items) do
				local btn = self.SuggestButtons[i]
				if not btn then
				...
				btn.LayoutOrder = i
				btn.Text = item.name .. (item.func and "()" or "")
				btn.TextColor3 = item.func and Settings.Theme.Syntax.FunctionName or Settings.Theme.Text
				btn.Visible = true
			end

			local shown = math.min(#items, 8)
			self.GuiElems.SuggestHolder.Size = UDim2.new(0,220,0,shown*rowH)
			list.CanvasSize = UDim2.new(0,0,0,#items*rowH)
			self:HighlightSuggestion()
		end

		funcs.HighlightSuggestion = function(self)
			for i,btn in ipairs(self.SuggestButtons) do
		...
		funcs.PositionSuggestions = function(self)
			...
			local frameH = self.Frame.AbsoluteSize.Y
		...
		funcs.UpdateSuggestions = function(self)
			...
			local line = self.Lines[self.CursorY+1] or ""
			local partial, base, sep = getContext(line, self.CursorX)

			-- need something typed, or a dot/colon context
			...
			local pool
			if sep then
				pool = (base and Suggest.libMap[base]) or Suggest.instanceMembers
			else
				pool = Suggest.globals
			end

			local matches = buildMatches(pool, partial)
			-- don't show if only match equals what's typed
		...
		funcs.AcceptSuggestion = function(self)
			local item = self.SuggestItems[self.SuggestIndex]
			if not item then self.Completing = false return end

			self.Completing = true
			local partial = self.SuggestPartial or ""

			-- delete the partial word already typed
			if #partial > 0 then
				local startX = self.CursorX - #partial
				self:DeleteRange({{startX, self.CursorY},{self.CursorX, self.CursorY}}, true, true)
			end

			local insert = item.name .. (item.func and "()" or "")
			self:AppendText(insert)
			if item.func then
				-- put cursor between the parens
	...
	Lib.Checkbox = (function()
		...
		local function initGui(self,frame)
			local checkbox = frame or create({
				...
				{4,"Frame",{BackgroundColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),BorderSizePixel=0,Name="filler",Parent={3},Position=UDim2.new(0,1,0,1),Size=UDim2.new(0,14,0,14),}},
	...
	Lib.BrickColorPicker = (function()
		...
		local function createGui(self)
			local gui = create({
				...
				{2,"Frame",{Active=true,BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Parent={1},Position=UDim2.new(0.40000000596046,0,0.40000000596046,0),Size=UDim2.new(0,337,0,380),}},
				{3,"TextButton",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Font=3,Name="MoreColors",Parent={2},Position=UDim2.new(0,5,1,-30),Size=UDim2.new(1,-10,0,25),Text="More Colors",TextColor3=Color3.new(1,1,1),TextSize=14,}},
	...
	Lib.ColorPicker = (function() -- TODO: Convert to newer class model
		...
		local function new()
			...
			local guiContents = create({
				{1,"Frame",{BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderSizePixel=0,ClipsDescendants=true,Name="Content",Position=UDim2.new(0,0,0,20),Size=UDim2.new(1,0,1,-20),}},
				...
				{4,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Blue",Parent={1},Position=UDim2.new(1,-63,0,255),Size=UDim2.new(0,52,0,16),}},
				{5,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),Font=3,Name="Input",Parent={4},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,50,0,16),Text="0",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
				...
				{18,"Frame",{BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,ClipsDescendants=true,Name="ColorSpaceFrame",Parent={1},Position=UDim2.new(1,-261,0,4),Size=UDim2.new(0,222,0,202),}},
				...
				{25,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Green",Parent={1},Position=UDim2.new(1,-63,0,233),Size=UDim2.new(0,52,0,16),}},
				{26,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),Font=3,Name="Input",Parent={25},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,50,0,16),Text="0",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
				...
				{39,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Hue",Parent={1},Position=UDim2.new(1,-180,0,211),Size=UDim2.new(0,52,0,16),}},
				{40,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),Font=3,Name="Input",Parent={39},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,50,0,16),Text="0",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
				...
				{53,"Frame",{BackgroundColor3=Color3.new(1,1,1),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Name="Preview",Parent={1},Position=UDim2.new(1,-260,0,211),Size=UDim2.new(0,35,1,-245),}},
				{54,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Red",Parent={1},Position=UDim2.new(1,-63,0,211),Size=UDim2.new(0,52,0,16),}},
				{55,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),Font=3,Name="Input",Parent={54},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,50,0,16),Text="0",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
				...
				{68,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Sat",Parent={1},Position=UDim2.new(1,-180,0,233),Size=UDim2.new(0,52,0,16),}},
				{69,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),Font=3,Name="Input",Parent={68},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,50,0,16),Text="0",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
				...
				{82,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Val",Parent={1},Position=UDim2.new(1,-180,0,255),Size=UDim2.new(0,52,0,16),}},
				{83,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),Font=3,Name="Input",Parent={82},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,50,0,16),Text="255",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
				...
				{96,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Font=3,Name="Cancel",Parent={1},Position=UDim2.new(1,-105,1,-28),Size=UDim2.new(0,100,0,25),Text="Cancel",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,}},
				{97,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Font=3,Name="Ok",Parent={1},Position=UDim2.new(1,-210,1,-28),Size=UDim2.new(0,100,0,25),Text="OK",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,}},
				{98,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Image="rbxassetid://1072518502",Name="ColorStrip",Parent={1},Position=UDim2.new(1,-30,0,5),Size=UDim2.new(0,13,0,200),}},
			...
			colorChoice.BorderColor3 = Settings.Theme.Outline2
			colorChoice.Text = ""
	...
	Lib.NumberSequenceEditor = (function()
		local function new() -- TODO: Convert to newer class model
			...
			local guiContents = create({
				{1,"Frame",{BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderSizePixel=0,ClipsDescendants=true,Name="Content",Position=UDim2.new(0,0,0,20),Size=UDim2.new(1,0,1,-20),}},
				{2,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Time",Parent={1},Position=UDim2.new(0,40,0,210),Size=UDim2.new(0,60,0,20),}},
				{3,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),ClipsDescendants=true,Font=3,Name="Input",Parent={2},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,58,0,20),Text="0",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
				...
				{5,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Font=3,Name="Close",Parent={1},Position=UDim2.new(1,-90,0,210),Size=UDim2.new(0,80,0,20),Text="Close",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,}},
				{6,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Font=3,Name="Reset",Parent={1},Position=UDim2.new(1,-180,0,210),Size=UDim2.new(0,80,0,20),Text="Reset",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,}},
				{7,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Font=3,Name="Delete",Parent={1},Position=UDim2.new(0,380,0,210),Size=UDim2.new(0,80,0,20),Text="Delete",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,}},
				{8,"Frame",{BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Name="NumberLineOutlines",Parent={1},Position=UDim2.new(0,10,0,20),Size=UDim2.new(1,-20,0,170),}},
				{9,"Frame",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),Name="NumberLine",Parent={1},Position=UDim2.new(0,10,0,20),Size=UDim2.new(1,-20,0,170),}},
				{10,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Value",Parent={1},Position=UDim2.new(0,170,0,210),Size=UDim2.new(0,60,0,20),}},
				...
				{12,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),ClipsDescendants=true,Font=3,Name="Input",Parent={10},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,58,0,20),Text="0",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
				{13,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Envelope",Parent={1},Position=UDim2.new(0,300,0,210),Size=UDim2.new(0,60,0,20),}},
				{14,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),ClipsDescendants=true,Font=3,Name="Input",Parent={13},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,58,0,20),Text="0",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
	...
	Lib.ColorSequenceEditor = (function() -- TODO: Convert to newer class model
		local function new()
			...
			local guiContents = create({
				{1,"Frame",{BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderSizePixel=0,ClipsDescendants=true,Name="Content",Position=UDim2.new(0,0,0,20),Size=UDim2.new(1,0,1,-20),}},
				{2,"Frame",{BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Name="ColorLine",Parent={1},Position=UDim2.new(0,10,0,5),Size=UDim2.new(1,-20,0,70),}},
				...
				{7,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Time",Parent={1},Position=UDim2.new(0,40,0,95),Size=UDim2.new(0,100,0,20),}},
				{8,"TextBox",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BackgroundTransparency=1,BorderColor3=Color3.new(0.37647062540054,0.37647062540054,0.37647062540054),ClipsDescendants=true,Font=3,Name="Input",Parent={7},Position=UDim2.new(0,2,0,0),Size=UDim2.new(0,98,0,20),Text="0",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
				...
				{10,"Frame",{BackgroundColor3=Color3.new(1,1,1),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Name="ColorBox",Parent={1},Position=UDim2.new(0,220,0,95),Size=UDim2.new(0,20,0,20),}},
				...
				{12,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Font=3,Name="Close",Parent={1},Position=UDim2.new(1,-90,0,95),Size=UDim2.new(0,80,0,20),Text="Close",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,}},
				{13,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Font=3,Name="Reset",Parent={1},Position=UDim2.new(1,-180,0,95),Size=UDim2.new(0,80,0,20),Text="Reset",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,}},
				{14,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Font=3,Name="Delete",Parent={1},Position=UDim2.new(0,280,0,95),Size=UDim2.new(0,80,0,20),Text="Delete",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,}},
...
["AnimationPreviewer"] = function()
--[[
	Animation Previewer App Module

	Spawns a rig inside a ViewportFrame (just like the 3D Viewer / Object viewer)
	and plays a selected Animation on it in a loop. The window is empty until you
	right-click an Animation in the Explorer and choose "Animation Previewer".
]]
...
local function main()
	...
	local window, viewportFrame, worldModel, pathLabel, settingsButton
	local camera, rig, track, currentAnim

	-- Build the preview rig by cloning the local player's character. This keeps
	-- the exact avatar (R6/R15) the game's animations are authored for, along
	-- with its Motor6Ds/Humanoid/Animator so the animation actually plays.
	local function buildRig()
		...
		-- Strip scripts so nothing runs / plays default idle animations inside
		-- the viewport; keep meshes, decals, motors and the humanoid.
	...
	AnimationPreviewer.StopPreview = function()
		if track then pcall(function() track:Stop() end) track = nil end
		if camera then pcall(function() camera:Destroy() end) camera = nil end
		rig = nil
		currentAnim = nil
		if worldModel then worldModel:ClearAllChildren() end
		AnimationPreviewer.IsViewing = false
		if window then window:SetTitle("Animation Previewer") end
		if pathLabel then pathLabel.Gui.Text = "" end
	end

	AnimationPreviewer.PreviewAnimation = function(anim)
		if not anim or not anim:IsA("Animation") then return end
		AnimationPreviewer.StopPreview()

		local newRig = buildRig()
		if not newRig then
			window:SetTitle("Animation Previewer - No character")
			pathLabel.Gui.Text = "Spawn/respawn your character to preview animations."
			window:Show()
			return
		end
		rig = newRig
		...
		-- Center the rig at the origin using its root part.
		local hrp = rig:FindFirstChild("HumanoidRootPart") or rig:FindFirstChild("Torso") or rig:FindFirstChildWhichIsA("BasePart")
		if hrp then
			rig.PrimaryPart = hrp
			pcall(function() rig:SetPrimaryPartCFrame(CFrame.new(0, 0, 0)) end)
			pcall(function() hrp.Anchored = true end)
		end

		-- Find (or create) an Animator to play through.
		local animator
		...
		if not animator then
			local ac = rig:FindFirstChildOfClass("AnimationController") or Instance.new("AnimationController")
			ac.Parent = rig
			animator = ac:FindFirstChildOfClass("Animator") or Instance.new("Animator")
			animator.Parent = ac
		end
		...
		window:SetTitle(anim.Name.." - Animation Previewer")
		pathLabel.Gui.Text = "path: " .. getPath(anim)
		window:Show()
		AnimationPreviewer.IsViewing = true

		-- WorldModel inside a ViewportFrame steps Animators automatically.
		...
		if not ok then
			pathLabel.Gui.Text = "Failed to load animation: "..tostring(err)
		end
	end

	AnimationPreviewer.ReplayAnimation = function()
		if currentAnim then
			AnimationPreviewer.PreviewAnimation(currentAnim)
		end
	end

	AnimationPreviewer.Init = function()
		window = Lib.Window.new()
		window:SetTitle("Animation Previewer")
		window:Resize(350, 250)
		AnimationPreviewer.Window = window
		...
		viewportFrame.InputBegan:Connect(function(input)
			if not AnimationPreviewer.EnableInputCamera then return end
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				dragging = true
				lastpos = input.Position
			elseif input.KeyCode == Enum.KeyCode.LeftShift then
				AnimationPreviewer.ZoomMultiplier = 10
			end
		end)
		...
		viewportFrame.InputEnded:Connect(function(input)
			...
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				dragging = false
			elseif input.KeyCode == Enum.KeyCode.LeftShift then
				AnimationPreviewer.ZoomMultiplier = 2
			end
		end)
		...
		viewportFrame.InputChanged:Connect(function(input)
			...
			if input.UserInputType == Enum.UserInputType.MouseWheel and hovering then
				distance = math.clamp(distance - (input.Position.Z * AnimationPreviewer.ZoomMultiplier), 0.1, math.huge)
			end
		end)

		RunService.RenderStepped:Connect(function(dt)
			if camera and rig and rig.PrimaryPart then
				if not dragging and AnimationPreviewer.AutoRotate then
					rotationY += AnimationPreviewer.RotationSpeed * dt * 60
				end
		...
		context:Register("STOP",{Name = "Stop Preview", OnClick = function()
			AnimationPreviewer.StopPreview()
		end})
		context:Register("REPLAY",{Name = "Replay Animation", OnClick = function()
			AnimationPreviewer.ReplayAnimation()
		end})
		context:Register("COPY_PATH",{Name = "Copy Animation Path", OnClick = function()
			if currentAnim and env.setclipboard then
				env.setclipboard(getPath(currentAnim))
			end
		end})
		context:Register("ENABLE_AUTO_ROTATE",{Name = "Enable Auto Rotate", OnClick = function()
			AnimationPreviewer.AutoRotate = true
		end})
		context:Register("DISABLE_AUTO_ROTATE",{Name = "Disable Auto Rotate", OnClick = function()
			AnimationPreviewer.AutoRotate = false
		end})
		context:Register("LOCK_CAM",{Name = "Lock Camera", OnClick = function()
			AnimationPreviewer.EnableInputCamera = false
		end})
		context:Register("UNLOCK_CAM",{Name = "Unlock Camera", OnClick = function()
			AnimationPreviewer.EnableInputCamera = true
		end})
		context:Register("EXIT",{Name = "Exit", OnClick = function()
			AnimationPreviewer.StopPreview()
			context:Hide()
		...
		local function ShowContext()
			...
			context:AddRegistered("STOP", not AnimationPreviewer.IsViewing)
			context:AddRegistered("REPLAY", not AnimationPreviewer.IsViewing)
			if env.setclipboard then context:AddRegistered("COPY_PATH", not AnimationPreviewer.IsViewing) end

			context:AddDivider()

			if AnimationPreviewer.AutoRotate then
				context:AddRegistered("DISABLE_AUTO_ROTATE")
			else
				context:AddRegistered("ENABLE_AUTO_ROTATE")
			end
			if AnimationPreviewer.EnableInputCamera then
				context:AddRegistered("LOCK_CAM")
			else
				context:AddRegistered("UNLOCK_CAM")
			end
...
local function main()
	...
	Properties.InitInputBox = function()
		inputBox = create({
			{1,"Frame",{BackgroundColor3=Color3.new(0.10980392156863,0.12156862745098,0.14901960784314),BorderSizePixel=0,Name="InputBox",Size=UDim2.new(0,200,0,22),Visible=false,ZIndex=2,}},
			{2,"TextBox",{BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BackgroundTransparency=1,BorderColor3=Color3.new(0.062745101749897,0.51764708757401,1),BorderSizePixel=0,ClearTextOnFocus=false,Font=3,Parent={1},PlaceholderColor3=Color3.new(0.69803923368454,0.69803923368454,0.69803923368454),Position=UDim2.new(0,3,0,0),Size=UDim2.new(1,-6,1,0),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=0,ZIndex=2,}},
		})
	...
	Properties.InitEntryStuff = function()
		Properties.EntryTemplate = create({
			{1,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Font=3,Name="Entry",Position=UDim2.new(0,1,0,1),Size=UDim2.new(0,250,0,22),Text="",TextSize=14,}},
			...
			{8,"Frame",{BackgroundColor3=Color3.new(0.10196078431373,0.11372549019608,0.13725490196078),BorderColor3=Color3.new(0.33725491166115,0.49019610881805,0.73725491762161),BorderSizePixel=0,Name="Line",Parent={7},Position=UDim2.new(0,-1,0,0),Size=UDim2.new(0,1,1,0),}},
			...
			{23,"Frame",{BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),Name="Slider",Parent={22},Position=UDim2.new(0,-4,0,-8),Size=UDim2.new(0,8,0,18),}},
			...
			{26,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderSizePixel=0,Font=3,Name="RowButton",Parent={1},Size=UDim2.new(1,0,1,0),Text="Add Attribute",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=0.10000000149012,Visible=false,}},
	...
	Properties.Init = function() -- TODO: MAKE BETTER
		local guiItems = create({
			...
			{5,"UICorner",{CornerRadius=UDim.new(0,2),Parent={3},}},
			{6,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Reset",Parent={3},Position=UDim2.new(1,-17,0,1),Size=UDim2.new(0,16,0,16),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,}},
			...
			{8,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Refresh",Parent={2},Position=UDim2.new(1,-20,0,1),Size=UDim2.new(0,18,0,18),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,Visible=false,}},
			...
			{10,"Frame",{BackgroundColor3=Color3.new(0.11764705882353,0.12941176470588,0.15686274509804),BorderSizePixel=0,Name="ScrollCorner",Parent={1},Position=UDim2.new(1,-16,1,-16),Size=UDim2.new(0,16,0,16),Visible=false,}},
...
local function main()
	...
	SaveInstance.Init = function()
		...
		ListFrame.TopImage = ""
		ListFrame.ScrollBarImageColor3 = Color3.fromRGB(64, 70, 82)
...
local function main()
	...
	-- Multi-tab + search state
	local Tabs = {}          -- { {name=, text=}, ... }
	local ActiveTab = 0
	local tabBar, tabButtons = nil, {}
	local SearchState = {matches = {}, cur = 0, open = false}
	local rebuildTabBar, switchTab, newTab, closeTab, runSearch, gotoMatch
	...
	ScriptViewer.Init = function()
		...
		-- Tab bar (row under the top buttons)
		tabBar = Instance.new("Frame")
		tabBar.BackgroundColor3 = Settings.Theme.Main2
		...
		tabScroll.Size = UDim2.new(1,-46,1,0)
		tabScroll.CanvasSize = UDim2.new(0,0,0,0)
		tabScroll.AutomaticCanvasSize = Enum.AutomaticSize.X
		tabScroll.ScrollBarThickness = 3
		tabScroll.ScrollingDirection = Enum.ScrollingDirection.X
		tabScroll.ScrollBarImageColor3 = Settings.Theme.Button
		...
		addTabBtn.AnchorPoint = Vector2.new(1,0)
		addTabBtn.Position = UDim2.new(1,-24,0,0)
		addTabBtn.Size = UDim2.new(0,22,1,0)
		addTabBtn.Text = "+"
		addTabBtn.TextSize = 20
		addTabBtn.Font = Enum.Font.SourceSansBold
		addTabBtn.TextColor3 = Settings.Theme.Text
		addTabBtn.Parent = tabBar
		addTabBtn.MouseButton1Click:Connect(function() newTab() end)

		-- Search toggle button (image button, top-right of tab bar)
		local searchToggle = Instance.new("ImageButton")
		searchToggle.BackgroundTransparency = 1
		searchToggle.AnchorPoint = Vector2.new(1,0)
		searchToggle.Position = UDim2.new(1,-2,0,2)
		searchToggle.Size = UDim2.new(0,16,0,16)
		-- Use RemoteSpy's icon from the LargeIcons sprite sheet (SpyIcon = index 10, 256x256 sheet, 32x32 tiles, NumX = 8)
		searchToggle.Image = "rbxassetid://100324125027397"
		searchToggle.ImageRectOffset = Vector2.new(64,32)
		searchToggle.ImageRectSize = Vector2.new(32,32)
		searchToggle.ScaleType = Enum.ScaleType.Fit
		searchToggle.ImageColor3 = Settings.Theme.Text
		searchToggle.Parent = tabBar

		-- Search overlay
		local searchBar = Instance.new("Frame")
		searchBar.BackgroundColor3 = Settings.Theme.Main2
		searchBar.BorderColor3 = Settings.Theme.Outline1
		searchBar.BorderSizePixel = 1
		searchBar.AnchorPoint = Vector2.new(1,0)
		searchBar.Position = UDim2.new(1,-8,0,44)
		searchBar.Size = UDim2.new(0,260,0,26)
		searchBar.Visible = false
		searchBar.ZIndex = 20
		searchBar.Parent = window.GuiElems.Content

		local searchInput = Instance.new("TextBox")
		searchInput.BackgroundColor3 = Settings.Theme.TextBox
		searchInput.BorderColor3 = Settings.Theme.Outline3
		searchInput.ClearTextOnFocus = false
		searchInput.Position = UDim2.new(0,4,0,4)
		searchInput.Size = UDim2.new(0,140,0,18)
		...
		prevBtn.ZIndex = 21
		prevBtn.Parent = searchBar

		local nextBtn = Instance.new("TextButton")
		nextBtn.BackgroundTransparency = 1
		nextBtn.Position = UDim2.new(0,228,0,4)
		...
		nextBtn.ZIndex = 21
		nextBtn.Parent = searchBar

		local closeSearch = Instance.new("TextButton")
		closeSearch.BackgroundTransparency = 1
		closeSearch.Position = UDim2.new(0,246,0,4)
		closeSearch.Size = UDim2.new(0,14,0,18)
		closeSearch.Text = "x"
		closeSearch.Font = Enum.Font.SourceSansBold
		closeSearch.TextSize = 15
		closeSearch.TextColor3 = Color3.fromRGB(255,120,120)
		closeSearch.ZIndex = 21
		closeSearch.Parent = searchBar

		-- Search logic
		gotoMatch = function(i)
			local m = SearchState.matches
			if #m == 0 then return end
			if i < 1 then i = #m elseif i > #m then i = 1 end
			SearchState.cur = i
			local match = m[i]
			-- highlight the found text and jump to it
			codeFrame.SelectionRange = {{match.col, match.line},{match.col + match.len, match.line}}
			codeFrame.CursorX = match.col
			codeFrame.CursorY = match.line
			codeFrame.ScrollV:ScrollTo(math.max(0, match.line - 4))
			codeFrame.ScrollH:ScrollTo(math.max(0, match.col - 4))
		...
		runSearch = function(query)
			...
			for lineIdx = 1, #lines do
				local text = (lines[lineIdx] or ""):lower()
				local init = 1
				while true do
					local s, e = text:find(ql, init, true)
					if not s then break end
					SearchState.matches[#SearchState.matches+1] = {line = lineIdx-1, col = s-1, len = (e - s + 1)}
					init = e + 1
				end
			end
		...
		searchToggle.MouseButton1Click:Connect(function()
			SearchState.open = not searchBar.Visible
			searchBar.Visible = SearchState.open
			if SearchState.open then
				searchInput:CaptureFocus()
				if searchInput.Text ~= "" then runSearch(searchInput.Text) end
			else
				codeFrame:ResetSelection()
			end
		end)
		...
		nextBtn.MouseButton1Click:Connect(function() gotoMatch(SearchState.cur + 1) end)
		prevBtn.MouseButton1Click:Connect(function() gotoMatch(SearchState.cur - 1) end)

		-- Tab management
		switchTab = function(index)
			if index == ActiveTab then return end
			if Tabs[ActiveTab] then
				Tabs[ActiveTab].text = codeFrame:GetText()
			end
			ActiveTab = index
			codeFrame:SetText(Tabs[index] and Tabs[index].text or "")
			rebuildTabBar()
			if SearchState.open and searchInput.Text ~= "" then runSearch(searchInput.Text) end
		end
		...
		rebuildTabBar = function()
			...
			for i, tab in ipairs(Tabs) do
				local holder = Instance.new("Frame")
				holder.BackgroundColor3 = (i == ActiveTab) and Settings.Theme.Main1 or Settings.Theme.Main2
				holder.BorderSizePixel = 0
				holder.Size = UDim2.new(0, 96, 1, 0)
				holder.LayoutOrder = i
				holder.Parent = tabScroll

				local nameBtn = Instance.new("TextButton")
				nameBtn.BackgroundTransparency = 1
				nameBtn.Position = UDim2.new(0,4,0,0)
				nameBtn.Size = UDim2.new(1,-18,1,0)
				nameBtn.Font = Enum.Font.SourceSans
				nameBtn.TextSize = 13
				nameBtn.TextColor3 = (i == ActiveTab) and Settings.Theme.Text or Color3.fromRGB(170,170,170)
				...
				closeBtn.Size = UDim2.new(0,14,1,0)
				...
				nameBtn.MouseButton1Click:Connect(function() switchTab(i) end)
				...
				nameBtn.MouseButton1Down:Connect(function()
					...
					if now - lastClick < 0.35 then
						...
						box.Position = UDim2.new(0,4,0,1)
						box.Size = UDim2.new(1,-18,1,-2)
				...
				tabButtons[#tabButtons+1] = holder
			end
		end

		newTab = function(name, text)
			if Tabs[ActiveTab] then Tabs[ActiveTab].text = codeFrame:GetText() end
			Tabs[#Tabs+1] = {name = name or ("Tab "..(#Tabs+1)), text = text or ""}
			ActiveTab = #Tabs
			codeFrame:SetText(Tabs[ActiveTab].text)
			rebuildTabBar()
			return ActiveTab
		end
		...
		closeTab = function(index)
			...
			if ActiveTab >= index then ActiveTab = math.max(1, ActiveTab - 1) end
			codeFrame:SetText(Tabs[ActiveTab].text)
			rebuildTabBar()
		end

		ScriptViewer.NewTab = newTab
		...
		save.TextColor3 = Color3.new(255,255,255)
		...
		dumpbtn.Position = UDim2.new(0.66,0,0,0)
		dumpbtn.Size = UDim2.new(0.34,0,0,20)
	...
	ScriptViewer.ViewScript = function(scr)
		...
		if ScriptViewer.NewTab then
			-- put the viewed script in its own named tab
			ScriptViewer.NewTab(tostring(scr.Name), source)
		else
			codeFrame:SetText(source)
		end
		window:Show()
...
--[[
	...
	Custom-built remote spy (no external loadstring). Logs every RemoteEvent /
	RemoteFunction call in BOTH directions:
	  - Outgoing (you -> server): colon-calls via __namecall AND dot-calls via
	    hookfunction on FireServer/InvokeServer.
	  - Incoming (server -> you): OnClientEvent / OnClientInvoke listeners on
	    every remote in the DataModel, including ones added later.
	Shows generated calling / listening code in a Dex+++ styled window.
]]
...
local function main()
	local RemoteSpy: {[string]: any} = {}

	local window, codeFrame, listFrame, listLayout, statusLabel
	local logs = {}
	local selectedEntry = nil
	local paused = false
	local LogLimit = 200
	local ignored = {}   -- [remote] = true, don't log
	local blocked = {}   -- [remote] = true, block from firing
	local hooked = false

	local theme = function() return Settings.Theme end

	local function getPath(obj)
		local ok, path = pcall(function()
			if obj.Parent == nil then return "nil --[[ "..obj.ClassName.." ]]" end
			if Explorer and Explorer.GetInstancePath then
				return Explorer.GetInstancePath(obj)
			end
			return obj:GetFullName()
		end)
		return ok and path or ("game --[[ "..tostring(obj).." ]]")
	end
	...
	local valueToString
	valueToString = function(v, depth, seen)
		depth = depth or 0
		seen = seen or {}
		local t = typeof(v)
		if t == "string" then
		...
		elseif t == "table" then
			...
			for k in pairs(v) do
				count = count + 1
				if typeof(k) ~= "number" then isArray = false end
			end
			...
			for k, val in pairs(v) do
				...
				if isArray then
					keyStr = ""
				elseif typeof(k) == "string" and k:match("^[%a_][%w_]*$") then
					keyStr = k.." = "
				else
					keyStr = "["..valueToString(k, depth+1, seen).."] = "
				end
				parts[#parts+1] = indent(depth+1)..keyStr..valueToString(val, depth+1, seen)
			end
			seen[v] = nil
			return "{\n"..table.concat(parts, ",\n").."\n"..indent(depth).."}"
		else
	...
	local function generateCode(entry)
		...
		local incoming = (entry.Direction == "Incoming")

		local argStrs = {}
		for i = 1, entry.ArgCount do
			argStrs[i] = valueToString(args[i], 0)
		end

		local header = ("-- Remote Spy | %s (%s)\n-- Remote: %s\n-- Method: %s\n-- %s: %s\n-- Time: %s\n\n"):format(
			entry.ClassName, incoming and "INCOMING (server -> you)" or "OUTGOING (you -> server)",
			remote and remote.Name or "?", method,
			incoming and "Received by" or "Called by", entry.Caller or "unknown", os.date("%H:%M:%S", entry.Time)
		)

		local argsBlock = ""
		if entry.ArgCount > 0 then
			argsBlock = "local args = {\n"
			local lines = {}
			for i = 1, entry.ArgCount do
				lines[i] = indent(1)..argStrs[i]
			end
			argsBlock = argsBlock..table.concat(lines, ",\n").."\n}\n\n"
		else
			argsBlock = "local args = {}\n\n"
		end
		...
		local callLine
		if incoming then
			-- Server fired this to you; show how to listen for / replicate it.
			if entry.ClassName == "RemoteFunction" then
				callLine = ("-- This was an OnClientInvoke request from the server.\n%s.OnClientInvoke = function(...)\n    local args = {...}\n    -- return a value here\nend"):format(path)
			else
				callLine = ("-- These args were sent to you by the server.\n%s.OnClientEvent:Connect(function(...)\n    local args = {...}\nend)"):format(path)
			end
		elseif method == "InvokeServer" or method == "invokeServer" then
			callLine = ("local result = %s:%s(unpack(args))\nprint(result)"):format(path, method)
		else
			callLine = ("%s:%s(unpack(args))"):format(path, method)
		end

		return header..argsBlock..callLine
	end

		local function selectLog(entry)
			selectedEntry = entry
			for _, e in ipairs(logs) do
				if e.Button then
					e.Button.BackgroundColor3 = (e == entry) and theme().ListSelection or theme().Button
				end
			end
			if entry and codeFrame then
				codeFrame:SetText(generateCode(entry))
			end
		end

		local function makeListEntry(entry)
			...
			local incoming = (entry.Direction == "Incoming")
			local arrow = incoming and "\u{2193} " or "\u{2191} "   -- down = incoming, up = outgoing
			nameLabel.Text = arrow..(entry.Remote and entry.Remote.Name or "?")
			...
			infoLabel.TextColor3 = incoming and Color3.fromRGB(120,190,255) or Color3.fromRGB(160,160,160)
			infoLabel.TextTruncate = Enum.TextTruncate.AtEnd
			local dirText = incoming and "IN" or "OUT"
			local isFunc = (entry.ClassName == "RemoteFunction")
			infoLabel.Text = dirText.." - "..(isFunc and "RemoteFunction" or "RemoteEvent").." - "..entry.ArgCount.." arg(s) - "..os.date("%H:%M:%S", entry.Time)
		...
		local function addLog(entry)
			...
			for i, e in ipairs(logs) do
				if e.Button then e.Button.LayoutOrder = #logs - i end
			end
			while #logs > LogLimit do
				local old = table.remove(logs, 1)
				if old.Button then old.Button:Destroy() end
				if selectedEntry == old then selectedEntry = nil end
			end
			if statusLabel then
				statusLabel.Text = "Logged: "..#logs..(paused and "  (PAUSED)" or "")
			end
		end

	-- The Actual Hook
	local FireMethods = {
		FireServer = "RemoteEvent", fireServer = "RemoteEvent",
		InvokeServer = "RemoteFunction", invokeServer = "RemoteFunction",
	}

	-- Shared logging helper. direction = "Outgoing" | "Incoming"
	local function record(remote, method, className, direction, args, n)
		if paused then return end
		-- Don't build up logs (and eat memory/CPU serializing args) unless the
		-- Remote Spy window is actually open. This keeps the passive hooks alive
		-- so nothing is missed the moment you open it, without lagging while closed.
		if not (window and window:IsContentVisible()) then return end
		if ignored[remote] then return end
		local caller
		if direction == "Incoming" then
			caller = "server (replicated)"
		else
			caller = "unknown"
			pcall(function()
				local gcs = env and env.getcallingscript
				local s = gcs and gcs()
				if s then caller = getPath(s) end
			end)
		end
		local entry = {
			Remote = remote,
	...
	-- Track which remotes we've already attached incoming listeners to.
	local incomingHooked = setmetatable({}, {__mode = "k"})

	-- Attach an OnClientEvent listener to a RemoteEvent / UnreliableRemoteEvent
	-- so we log everything the SERVER fires at us (not fired by us at all).
	local function watchIncoming(remote)
		if incomingHooked[remote] then return end
		local cls = remote.ClassName
		if cls == "RemoteEvent" or cls == "UnreliableRemoteEvent" then
			incomingHooked[remote] = true
			pcall(function()
				remote.OnClientEvent:Connect(function(...)
					record(remote, "OnClientEvent", cls, "Incoming", {...}, select("#", ...))
				end)
			end)
		elseif cls == "RemoteFunction" then
			-- OnClientInvoke is a single callback, not an event. Only wrap it if
			-- one isn't already in use, and preserve its return value.
			incomingHooked[remote] = true
			pcall(function()
				local existing = remote.OnClientInvoke
				remote.OnClientInvoke = function(...)
					record(remote, "OnClientInvoke", cls, "Incoming", {...}, select("#", ...))
	...
	-- Scan the whole DataModel for remotes and watch newly added ones.
	local function scanIncoming()
		pcall(function()
	...
	local function setupHook()
		...
		local hookmetamethod = env and env.hookmetamethod
		local getnamecallmethod = (env and env.getnamecallmethod) or getnamecallmethod
		if not hookmetamethod or not getnamecallmethod then
			return false, "Your executor is missing hookmetamethod/getnamecallmethod."
		end

		-- 1) OUTGOING colon-calls: remote:FireServer(...) / remote:InvokeServer(...)
		local oldNamecall
		oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
			local method = getnamecallmethod()
			local cls = FireMethods[method]
			if cls and typeof(self) == "Instance" then
				-- block if requested
				...
				record(self, method, self.ClassName, "Outgoing", {...}, select("#", ...))
			end
			return oldNamecall(self, ...)
		end)

		-- 2) OUTGOING dot-calls: remote.FireServer(remote, ...) — these skip __namecall,
		--    so hook the actual methods if the executor supports hookfunction.
		local hookfunction = env and (env.hookfunction or env.replaceclosure or env.hookfunc)
		if hookfunction then
			pcall(function()
				...
				oldFire = hookfunction(dummyEvent.FireServer, function(self, ...)
					if typeof(self) == "Instance" and (self.ClassName == "RemoteEvent" or self.ClassName == "UnreliableRemoteEvent") then
						if blocked[self] then return end
						record(self, "FireServer", self.ClassName, "Outgoing", {...}, select("#", ...))
					end
					return oldFire(self, ...)
				end)

				oldInvoke = hookfunction(dummyFunc.InvokeServer, function(self, ...)
					if typeof(self) == "Instance" and self.ClassName == "RemoteFunction" then
						if blocked[self] then return nil end
						record(self, "InvokeServer", self.ClassName, "Outgoing", {...}, select("#", ...))
					end
		...
		-- 3) INCOMING: server -> you (OnClientEvent / OnClientInvoke). No executor
		--    function needed, so this works even without hookfunction.
		scanIncoming()
	...
	RemoteSpy.Init = function()
		...
		listContainer.BackgroundColor3 = theme().Main2
		listContainer.BorderColor3 = theme().Outline1
		listContainer.BorderSizePixel = 1
		listContainer.Position = UDim2.new(0, 0, 0, 22)
		listContainer.Size = UDim2.new(0, 240, 1, -44)
		listContainer.Parent = window.GuiElems.Content
		...
		scroll.ScrollBarImageColor3 = theme().Button
		...
		-- Right: code view
		codeFrame = Lib.CodeFrame.new()
		codeFrame.Frame.Position = UDim2.new(0, 244, 0, 22)
		codeFrame.Frame.Size = UDim2.new(1, -244, 1, -44)
		codeFrame.Frame.Parent = window.GuiElems.Content
		codeFrame:SetText("-- Remote Spy ready.\n-- Logs ALL remotes: outgoing (you -> server) AND incoming (server -> you).\n-- Up arrow = outgoing, Down arrow = incoming.\n-- Fire or receive a remote and it will appear on the left.\n-- Click an entry to view its generated code.")

		-- Top status
		statusLabel = Instance.new("TextLabel")
		statusLabel.BackgroundTransparency = 1
		statusLabel.Position = UDim2.new(0, 4, 0, 1)
		statusLabel.Size = UDim2.new(1, -8, 0, 20)
		statusLabel.Font = Enum.Font.SourceSans
		statusLabel.TextSize = 14
		statusLabel.TextXAlignment = Enum.TextXAlignment.Left
		statusLabel.TextColor3 = theme().Text
		statusLabel.Text = "Logged: 0"
		statusLabel.Parent = window.GuiElems.Content

		-- Bottom toolbar
		local function makeBtn(text, xScale, xOff, wScale, wOff)
			local b = Instance.new("TextButton")
			...
			b.Position = UDim2.new(xScale, xOff, 1, -2)
			b.Size = UDim2.new(wScale, wOff, 0, 20)
			b.Parent = window.GuiElems.Content
			-- subtle hover feedback since bg is transparent
			b.MouseEnter:Connect(function() b.TextColor3 = theme().ListSelection end)
			b.MouseLeave:Connect(function() b.TextColor3 = theme().Text end)
			return b
		end

		local pauseBtn = makeBtn("Pause", 0, 0, 0.2, -2)
		pauseBtn.MouseButton1Click:Connect(function()
			paused = not paused
			pauseBtn.Text = paused and "Resume" or "Pause"
			if statusLabel then statusLabel.Text = "Logged: "..#logs..(paused and "  (PAUSED)" or "") end
		end)

		local clearBtn = makeBtn("Clear", 0.2, 2, 0.2, -4)
		clearBtn.MouseButton1Click:Connect(function()
			for _, e in ipairs(logs) do if e.Button then e.Button:Destroy() end end
			logs = {}
			selectedEntry = nil
			codeFrame:SetText("-- Cleared.")
			if statusLabel then statusLabel.Text = "Logged: 0"..(paused and "  (PAUSED)" or "") end
		end)

		local copyBtn = makeBtn("Copy Code", 0.4, 2, 0.2, -4)
		copyBtn.MouseButton1Click:Connect(function()
			if env.setclipboard and codeFrame then
				env.setclipboard(codeFrame:GetText())
			end
		end)

		local blockBtn = makeBtn("Block/Unblock", 0.6, 2, 0.2, -4)
		blockBtn.MouseButton1Click:Connect(function()
			local entry = selectedEntry
			if entry and entry.Remote then
				blocked[entry.Remote] = not blocked[entry.Remote]
				blockBtn.Text = blocked[entry.Remote] and "Unblock" or "Block/Unblock"
			end
		end)

		local ignoreBtn = makeBtn("Ignore/Unignore", 0.8, 2, 0.2, -2)
		ignoreBtn.MouseButton1Click:Connect(function()
			local entry = selectedEntry
			if entry and entry.Remote then
				ignored[entry.Remote] = not ignored[entry.Remote]
				ignoreBtn.Text = ignored[entry.Remote] and "Unignore" or "Ignore/Unignore"
			end
		end)

		-- Start the hook
		local ok, err = setupHook()
		if not ok and err then
			codeFrame:SetText("-- Remote Spy could not start.\n-- "..tostring(err))
		end
	end

	RemoteSpy.OpenRemoteSpy = function()
		if not window then RemoteSpy.Init() end
		window:ShowAndFocus()
	end

	RemoteSpy.HideRemoteSpy = function()
		if window then window:Hide() end
	end

	-- Build window + start hook immediately so calls are captured from load
	pcall(RemoteSpy.Init)
...
-- Main vars
local Main, Explorer, Properties, ScriptViewer, Console, SaveInstance, ModelViewer, AnimationPreviewer, SettingsWindow--[[, SecretServicePanel]], DefaultSettings, Notebook, Serializer, Lib local ggv = getgenv or nil
local API, RMD
...
DefaultSettings = (function()
	...
	return {
		...
		Theme = {
			...
			Main1 = rgb(46,50,58),
			Main2 = rgb(40,44,52),
			Outline1 = rgb(28,31,38), -- Mainly frames
			Outline2 = rgb(52,57,66), -- Mainly button
			Outline3 = rgb(26,29,35), -- Mainly textbox
			TextBox = rgb(33,37,44),
			Menu = rgb(30,33,40),
			ListSelection = rgb(11,90,175),
			Button = rgb(52,57,68),
			ButtonHover = rgb(62,68,80),
			ButtonPress = rgb(38,42,50),
			Highlight = rgb(66,72,85),
			Text = rgb(255,255,255),
			PlaceholderText = rgb(120,126,140),
		...
		ThemePreset = "Modern", -- "Modern" (blue-gray, default) or "Dark" (classic neutral gray)
		ClassIcon = "NewDark",
...
-- Theme presets. Each entry is a flat set of color keys that get copied into
-- Settings.Theme by Main.ApplyThemePreset. Syntax colors / icon maps / _Recurse
-- are intentionally left untouched so only the UI chrome changes.
local ThemePresets = (function()
	local rgb = Color3.fromRGB
	return {
		-- Current blue-gray look (matches DefaultSettings.Theme, the default).
		Modern = {
			Main1 = rgb(46,50,58),
			Main2 = rgb(40,44,52),
			Outline1 = rgb(28,31,38),
			Outline2 = rgb(52,57,66),
			Outline3 = rgb(26,29,35),
			TextBox = rgb(33,37,44),
			Menu = rgb(30,33,40),
			ListSelection = rgb(11,90,175),
			Button = rgb(52,57,68),
		...
		-- Classic neutral gray look. Keeps the blue selection highlight for readability.
		Dark = {
			Main1 = rgb(53,53,53),
			Main2 = rgb(46,46,46),
			Outline1 = rgb(33,33,33),
			Outline2 = rgb(60,60,60),
			...
			ButtonPress = rgb(45,45,45),
			Highlight = rgb(75,75,75),
			Text = rgb(255,255,255),
			PlaceholderText = rgb(150,150,150),
			Important = rgb(255,0,0),
		},
	}
end)()
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","Console","SaveInstance","ModelViewer","AnimationPreviewer","SettingsWindow","ChatLogger","AudioLogger","RemoteSpy"}
	Main.Elevated = false
	...
	Main.LoadModules = function()
		...
		SaveInstance = Apps.SaveInstance
		ModelViewer = Apps.ModelViewer
		AnimationPreviewer = Apps.AnimationPreviewer
		Notebook = Apps.Notebook
		SettingsWindow = Apps.SettingsWindow
	...
	Main.InitEnv = function()
		...
		env.getnamecallmethod = getnamecallmethod or get_namecall_method
			or (getgenv and (getgenv().getnamecallmethod or getgenv().get_namecall_method))
			or (rawget(getfenv(), "getnamecallmethod"))
		env.getrawmetatable = getrawmetatable
		env.setreadonly = setreadonly or make_writeable
		env.newcclosure = newcclosure
		env.iscclosure = iscclosure
	...
	Main.ApplyThemePreset = function(name)
		local preset = ThemePresets[name] or ThemePresets.Modern
		if not Settings.Theme then Settings.Theme = {} end
		-- Only copy the flat color keys; leave Syntax, icon maps and _Recurse alone.
	...
	Main.LoadSettings = function()
		local s, data = pcall(env.readfile or error, "DexPlusPlusPlusSettings.json")
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{2,"Frame",{Active=true,BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Name="Main",Parent={1},Position=UDim2.new(0.5,-175,0.5,-100),Size=UDim2.new(0,350,0,200),}},
			{3,"Frame",{BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderSizePixel=0,ClipsDescendants=true,Name="Holder",Parent={2},Size=UDim2.new(1,0,1,0),}},
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-190,0,15),Size=UDim2.new(0,100,0,50),Text="DexC",TextColor3=Color3.new(1,1,1),TextSize=50,TextTransparency=1,}},
			...
			{8,"Frame",{BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Name="ProgressBar",Parent={3},Position=UDim2.new(0,110,0,145),Size=UDim2.new(0,0,0,4),}},
			{9,"Frame",{BackgroundColor3=Color3.new(0.2392156869173,0.56078433990479,0.86274510622025),BorderSizePixel=0,Name="Bar",Parent={8},Size=UDim2.new(0,0,1,0),}},
			{10,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="rbxassetid://2764171053",ImageColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),Parent={8},ScaleType=1,Size=UDim2.new(1,0,1,0),SliceCenter=Rect.new(2,2,254,254),}},
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "Close" or "DexC"
	...
	Main.CreateMainGui = function()
		local gui = create({
			...
			{2,"TextButton",{AnchorPoint=Vector2.new(0.5,0),AutoButtonColor=false,BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),BorderSizePixel=0,Font=4,Name="OpenButton",Parent={1},Position=UDim2.new(0.5,0,0,2),Size=UDim2.new(0,55,0,32),Text="DexC",TextColor3=Color3.new(1,1,1),TextSize=16,TextTransparency=0.20000000298023,}},
			{3,"UICorner",{CornerRadius=UDim.new(0,4),Parent={2},}},
			{4,"Frame",{AnchorPoint=Vector2.new(0.5,0),BackgroundColor3=Color3.new(0.15686274509804,0.17254901960784,0.20392156862745),ClipsDescendants=true,Name="MainFrame",Parent={2},Position=UDim2.new(0.5,0,1,-4),Size=UDim2.new(0,224,0,200),}},
			{5,"UICorner",{CornerRadius=UDim.new(0,4),Parent={4},}},
			{6,"Frame",{BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),Name="BottomFrame",Parent={4},Position=UDim2.new(0,0,1,-24),Size=UDim2.new(1,0,0,24),}},
			{7,"UICorner",{CornerRadius=UDim.new(0,4),Parent={6},}},
			{8,"Frame",{BackgroundColor3=Color3.new(0.18039215686275,0.19607843137255,0.22745098039216),BorderSizePixel=0,Name="CoverFrame",Parent={6},Size=UDim2.new(1,0,0,4),}},
			{9,"Frame",{BackgroundColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),BorderSizePixel=0,Name="Line",Parent={8},Position=UDim2.new(0,0,0,-1),Size=UDim2.new(1,0,0,1),}},
			...
			{14,"ScrollingFrame",{Active=true,AnchorPoint=Vector2.new(0.5,0),BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,BorderColor3=Color3.new(0.09411764705882,0.10588235294118,0.12941176470588),BorderSizePixel=0,Name="AppsFrame",Parent={4},Position=UDim2.new(0.5,0,0,0),ScrollBarImageColor3=Color3.new(0,0,0),ScrollBarThickness=4,Size=UDim2.new(0,222,1,-25),}},
			...
			{18,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392156862745,0.22352941176471,0.26666666666667),BorderSizePixel=0,Font=3,Name="Main",Parent={17},Size=UDim2.new(1,0,0,60),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
		...
		Main.CreateApp({Name = "Remote Spy", IconMap = Main.LargeIcons, Icon = "SpyIcon", Open = false, OnClick = function(open)
			if open then
				Apps.RemoteSpy.OpenRemoteSpy()
			else
				Apps.RemoteSpy.HideRemoteSpy()
			end
		end})
		...
		Main.CreateApp({Name = "Animation Previewer", IconMap = Main.LargeIcons, Icon = "AnimationPreviewer", Window = AnimationPreviewer.Window})
	...
	Main.Init = function()
		...
		Main.ApplyThemePreset(Settings.ThemePreset) -- apply selected theme preset over the loaded theme
		...
		Main.LargeIcons = Lib.IconMap.new("rbxassetid://133857186491158",256,256,32,32)
		Main.LargeIcons:SetDict({
			Explorer = 0, Properties = 1, Script_Viewer = 2, Obfuscator_Icon = 9, SpyIcon = 10, LogIcon = 11, IYicon = 12, AudioIcon = 14, Watcher = 3, Output = 4, ScriptEdit = 5, Book = 6, Executor = 7, Object = 8, AnimationPreviewer = 15,
		})
